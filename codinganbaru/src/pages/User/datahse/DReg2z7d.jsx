...
import { faHouse, faFileContract, faHelmetSafety, faArrowRightFromBracket, 
  faBell, faSun, faMagnifyingGlass, faCalendarAlt, faIdCard, faCarAlt } from '@fortawesome/free-solid-svg-icons';
...
const DReg2z7d = () => {
  ...
  // Function to get link class name
  ...
  const getDaysDifference = (endDate) => {
    if (!endDate) return null; // Return null for empty dates
  ...
  const getFilteredData = (data, searchTerm, filterType, dateField) => {
    ...
    // Show all data if filter is "all" 
    if (filterType === 'all') {
      // If there's a search term, filter based on search
      if (searchTerm) {
    ...
    let statusFiltered = data.filter(employee => {
      // If filterType is not 'all', filter based on date
      if (!employee[dateField]) {
        // For 'nonactive', also show those without a date
    ...
    // Filter based on search if there is a search term
  ...
  const getHSECounts = () => {
    ...
    // Total all employees, including those without HSE Passport dates
    counts.total = backendData.length;

    // Count employees without HSE
    const noHseCount = backendData.filter(employee => !employee.akhir_hsepassport).length;
    
    // Add those without HSE to nonactive
  ...
  const getSIMLCounts = () => {
    ...
    // Total all employees, including those without SIM dates
    counts.total = backendData.length;

    // Count employees without SIM
    const noSimlCount = backendData.filter(employee => !employee.akhir_siml).length;
    
    // Add those without SIM to nonactive
  ...
  const mcuColumns = [
    ...
    { name: 'Employee Name', selector: row => row.nama_lengkap || '', sortable: true },
    { name: 'Position', selector: row => row.jabatan || '', sortable: true, width: "300px" },
    { name: 'MCU Vendor', selector: row => row.vendor_mcu || '', sortable: true, width: "300px" },
  ...
  const customStyles = {
    table: {
      style: {
        width: '100%',
        tableLayout: 'auto', // Let the browser determine the table layout automatically
      }
    },
    rows: { 
      style: { 
        minHeight: '60px',
        width: '100%' // Ensure rows take up the full available width
      },
    ...
    headCells: {
      style: {
        ...
        fontWeight: 'bold',
        color: '#FFFFFF',
        whiteSpace: 'normal', // Allow text to wrap if too long
        overflow: 'visible', // Don't hide overflow
        textOverflow: 'clip', // Don't truncate text
        wordWrap: 'break-word', // Allow long words to be broken
      },
      activeSortStyle: {
    ...
    cells: { 
      style: { 
        paddingLeft: '16px', 
        paddingRight: '16px',
        wordBreak: 'break-word', // Allow long words to be broken
        whiteSpace: 'normal', // Allow text to wrap if too long
      } 
    },
    pagination: {
      style: {
        borderTopStyle: 'none', // Remove top border on pagination
        width: '100%',
      },
    }
  };
  ...
  const tableWrapperStyle = {
    width: '100%',
    overflowX: 'auto', // Add horizontal scroll if table is too wide
    ...
    <div className='bg-(--background-tar-color)'> 
      <div className="flex">
          ...
          <div className='flex sticky top-0'>
            <aside className="w-70 bg-white text-black-70 p-4 flex flex-col h-screen overflow-y-auto">
              ...
              <nav className="mt-8 space-y-4">
                {/* Link Dashboard */}
                <Link to="/user-reg2z7d" className={getLinkClassName('/user-reg2z7d')}>
                  <FontAwesomeIcon icon={faHouse} className='text-lg'/>
                  <span className='text-sm'>Dashboard</span>
                </Link>
                
                {/* Link Contract */}
                <Link to="/contract-reg2z7d" className={getLinkClassName('/contract-reg2z7d')}>
                  <FontAwesomeIcon icon={faFileContract} className='text-lg'/>
                  <span className='text-sm'>Contract</span>
                </Link>
                
                {/* Link HSE */}
                <Link to="/data-hse-reg2z7d" className={getLinkClassName('/data-hse-reg2z7d')}>
        ...
        <div className="flex-1 flex flex-col h-screen ml-68 bg-(--background-tar-color) overflow-y-auto px-6 py-3">
          ...
          {/* Back button */}
          <div className="mb-4">
            <Link to='/data-hse-reg2z7d' className="inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition">
              &lt; Back
            </Link>
          </div>
          ...
          <h1 className="text-2xl font-bold mb-4 mt-2 text-center">Pertamina EP Regional 2 Zona 7 Devplan HSE View</h1>
                ...
                <div className="bg-white shadow rounded-lg p-6 mb-6">
                  ...
                  <div style={tableWrapperStyle}>
                    <DataTable 
                      ...
                      striped
                      responsive
                      defaultSortFieldId={1} // Default sort by MCU Status (1st column, index starts at 1)
                ...
                <div className="bg-white shadow rounded-lg p-6 mb-6">
                  ...
                  <div style={tableWrapperStyle}>
                    <DataTable 
                      ...
                      striped
                      responsive
                      defaultSortFieldId={1} // Default sort by HSE Status (1st column)
                      defaultSortAsc={true}  // Ascending order (lower priority values first)
                      noDataComponent={
                        <div className="p-4 text-center text-gray-500">
                ...
                <div className="bg-white shadow rounded-lg p-6 mb-6">
                  ...
                  <div style={tableWrapperStyle}>
                    <DataTable 
                      ...
                      striped
                      responsive
                      defaultSortFieldId={1} // Default sort by SIML Status (1st column)
                      defaultSortAsc={true}  // Ascending order (lower priority values first)
                      noDataComponent={
                        <div className="p-4 text-center text-gray-500">
