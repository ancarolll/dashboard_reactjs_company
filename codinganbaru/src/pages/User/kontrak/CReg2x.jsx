...
import { faHouse, faFileContract, faShieldHalved, faArrowRightFromBracket, faBell, faSun, 
        faMagnifyingGlass, faFileAlt, faDownload, faUpload } from '@fortawesome/free-solid-svg-icons';
...
const CReg2x = () => {
  ...
  const fetchDataFromBackend = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`${API_BASE_URL}/regional2x/view`, {
        headers: {
      ...
      try {
        const fallbackResponse = await axios.get(`${API_BASE_URL}/regional2x/users`, {
          headers: {
            'Accept': 'application/json'
          },
          timeout: 10000
  ...
  const filteredRecords = useMemo(() => {
    ...
    const searchFiltered = statusFiltered.filter(row => {
      ...
      const paketTender = row.paket_tender || '';
      ...
      return namaLengkap.toLowerCase().includes(searchValue) || 
            ...
            paketTender.toLowerCase().includes(searchValue);
    });
