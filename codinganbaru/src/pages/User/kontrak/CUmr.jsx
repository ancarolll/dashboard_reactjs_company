...
import { faHouse, faFileContract, faHelmetSafety, faArrowRightFromBracket, faBell, faSun, faMagnifyingGlass, faFileAlt, faUpload, faDownload, faImage, faLink
...
const CUmr = () => {
    ...
    try {
      ...
      if (response.data) {
        if (Array.isArray(response.data)) {
          ...
          setError("The data received from the server is invalid");
          setBackendData([]);
        }
      } else {
      ...
      let errorMessage = "Failed to retrieve data from the server";
      if (err.response) {
  ...
  // Filter data based on search term with priority sorting
  ...
  const StatBox = () => {
    ...
    const totalEmployees = backendData.filter(user => getDaysDifference(user.kontrak_akhir) >= 0).length;
    const dueDateUsers = backendData.filter(user => {
      const days = getDaysDifference(user.kontrak_akhir);
      return days >= 0 && days <= 14;
    }).length;
    const call2Users = backendData.filter(user => {
      const days = getDaysDifference(user.kontrak_akhir);
      return days > 14 && days <= 30;
    }).length;
    const call1Users = backendData.filter(user => {
      const days = getDaysDifference(user.kontrak_akhir);
      return days > 30 && days <= 45;
    }).length;
  ...
  const columns = [
    ...
    { name: 'Employee Name', selector: row => row.nama_lengkap_karyawan || '', sortable: true, width: "250px" },
    ...
    <div className='bg-(--background-tar-color)'> 
      <div className="flex">
          ...
          <div className='flex sticky top-0'>
            <aside className="w-70 bg-white text-black-70 p-4 flex flex-col h-screen overflow-y-auto">
              ...
              <nav className="mt-8 space-y-4">
                {/* Link Dashboard */}
                <Link to="/user-urp" className={getLinkClassName('/user-urp')}>
                  <FontAwesomeIcon icon={faHouse} className='text-lg'/>
                  <span className='text-sm'>Dashboard</span>
                </Link>
                
                {/* Link Contract */}
                <Link to="/contract-urp" className={getLinkClassName('/contract-urp')}>
                  <FontAwesomeIcon icon={faFileContract} className='text-lg'/>
                  <span className='text-sm'>Contract</span>
                </Link>
                
                {/* Link HSE */}
                <Link to="/data-hse-urp" className={getLinkClassName('/data-hse-urp')}>
        ...
        <div className="flex-1 flex flex-col h-screen ml-68 bg-(--background-tar-color) overflow-y-auto px-6 py-3">
          ...
          {/* Back button */}
          <div className="mb-4">
            <Link to='/user-urp' className="inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition">
              &lt; Back
            </Link>
          </div>
