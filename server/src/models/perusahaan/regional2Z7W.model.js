...
import fs from 'fs';
import { Regional2Z7WContractHistory } from "./regional2Z7W_contract_history.model.js";
...
export const Regional2Z7W = {
    ...
    // Definisi kolom numeric (sesuaikan dengan struktur tabel Regional2Z7W)
    ...
    // Bersihkan data
    const cleanData = Regional2Z7W._cleanData(dataWithoutId);
    ...
    const sql = `
        INSERT INTO project_pertamina_ep_regional2zona7_wopdm (${filteredKeys.join(', ')})
        VALUES (${placeholders})
        RETURNING *
    `;
    ...
    const cleanData = Regional2Z7W._cleanData(data);
    ...
    const fileFields = [
        ...
        // New document fields for regional2Z7W
    ...
    const sql = `
        UPDATE project_pertamina_ep_regional2zona7_wopdm
        SET ${setClause}
    ...
    // Buat query SQL dinamis
    const sql = `
        SELECT * FROM project_pertamina_ep_regional2zona7_wopdm
        ${whereClause}
        ORDER BY ${orderBy}
    `;
    ...
    // Buat query SQL dinamis
    const sql = `
        SELECT ${columnsClause} FROM project_pertamina_ep_regional2zona7_wopdm
        ${whereClause}
        ORDER BY ${orderBy}
    `;
    ...
    // Buat query SQL dinamis
    const sql = `
        DELETE FROM project_pertamina_ep_regional2zona7_wopdm
        ${whereClause}
        RETURNING *
    `;
    ...
    checkDocumentColumns: async () => {
        ...
        WHERE table_name = 'project_pertamina_ep_regional2zona7_wopdm' 
        AND column_name IN (
            'cv_filename', 'cv_filepath', 'cv_mimetype', 'cv_filesize',
            'ijazah_filename', 'ijazah_filepath', 'ijazah_mimetype', 'ijazah_filesize',
            'sertifikat_filename', 'sertifikat_filepath', 'sertifikat_mimetype', 'sertifikat_filesize', 
    ...
    try {
        ...
        try {
            ...
            const normalizedData = Regional2Z7W.validateAndNormalizeUserData(userData);
            
            // Siapkan query untuk insert
            const { sql, values } = Regional2Z7W._createInsertQuery(normalizedData);
    ...
    addDocumentColumns: async () => {
        try {
        const hasColumns = await Regional2Z7W.checkDocumentColumns();
        ...
        const sql = `
        ALTER TABLE project_pertamina_ep_regional2zona7_wopdm
    ...
    try {
        ...
        SELECT EXISTS (
            ...
            AND table_name = 'project_pertamina_ep_regional2zona7_wopdm'
        );
        `;
        const result = await pool.query(sql);
    ...
    resetSequence: async () => {
        ...
        const sequenceQuery = `
        SELECT pg_get_serial_sequence('project_pertamina_ep_regional2zona7_wopdm', 'id') as seq_name;
        `;
        const sequenceResult = await pool.query(sequenceQuery);
        const sequenceName = sequenceResult.rows[0]?.seq_name;
        
        if (!sequenceName) {
        console.error('Cannot find sequence for project_pertamina_ep_regional2zona7_wopdm.id');
        return false;
        }
        
        // Dapatkan nilai maksimal ID yang ada
        const maxIdQuery = `SELECT COALESCE(MAX(id), 0) + 1 as next_id FROM project_pertamina_ep_regional2zona7_wopdm`;
    ...
    uploadDocument: async (userId, docType, fileInfo) => {
        ...
        // Validate document type (including new document types for regional2Z7W)
        const validDocTypes = [
          'cv', 'ijazah', 'sertifikat', 'pkwt', 
          'no_ktp', 'no_kk', 'no_npwp', 'bpjs_kesehatan_karyawan', 
          'bpjstk', 'no_rekening'
        ];
        ...
        await Regional2Z7W.addDocumentColumns();
        
        // Pertama periksa apakah user dengan ID ini ada
        const checkUserSql = `SELECT id FROM project_pertamina_ep_regional2zona7_wopdm WHERE id = $1`;
        ...
        const querySql = `
        SELECT ${docType}_filepath FROM project_pertamina_ep_regional2zona7_wopdm
        WHERE id = $1
        `;
    ...
    getDocumentInfo: async (userId, docType) => {
        ...
        const sql = `
            ...
            FROM project_pertamina_ep_regional2zona7_wopdm 
            WHERE id = $1
        `;
    ...
    checkTableStructure: async () => {
        ...
        const tableExists = await Regional2Z7W.checkTableExists();
        
        if (!tableExists) {
        console.error("Table project_pertamina_ep_regional2zona7_wopdm does not exist!");
    ...
    getAll: async () => {
        ...
        const allRows = await pool.query('SELECT id, nama_lengkap, kontrak_akhir, sebab_na FROM project_pertamina_ep_regional2zona7_wopdm LIMIT 10');
        ...
        if (error.code === '42P01') {
        console.error("Table project_pertamina_ep_regional2zona7_wopdm doesn't exist");
        return [];
        }
        throw error;
    }
    ...
    try {
        ...
        const userSql = `SELECT * FROM project_pertamina_ep_regional2zona7_wopdm WHERE id = $1`;
        ...
        // Update data
        const { sql, values } = Regional2Z7W._createUpdateQuery(id, updateData);
    ...
    const { sql, values } = Regional2Z7W._createInsertQuery(userData);
    ...
    try {
        ...
        const { sql, values } = Regional2Z7W._createSelectQuery({ id });
        const result = await pool.query(sql, values);
        return result.rows[0];
    } catch (error) {
    ...
    try {
        ...
        const oldData = await Regional2Z7W.getUserById(id);
        if (!oldData) {
        ...
        const cleanedUserData = Regional2Z7W._cleanData(userData);
        ...
        // Update data user
        const { sql, values } = Regional2Z7W._createUpdateQuery(id, cleanedUserData);
        const result = await pool.query(sql, values);
    
        // Jika kontrak berubah, simpan history
        if (isKontrakChanged) {
        await Regional2Z7W.saveKontrakHistory(id, oldData, cleanedUserData);
        }
    ...
    try {
        ...
        // Periksa apakah tabel history_regional2z7w ada
        const sql = `
        SELECT EXISTS (
            SELECT FROM information_schema.tables 
            WHERE table_schema = 'public' 
            AND table_name = 'history_regional2z7w'
        );
        `;
        const tableExists = await pool.query(sql);
        
        if (!tableExists.rows[0].exists) {
        console.error("Table history_regional2z7w does not exist!");
        // Jika tabel tidak ada, buat log tapi jangan gagalkan operasi utama
        console.log("Cannot save contract history as history_regional2z7w table doesn't exist");
        return null;
        }
        ...
        const historySql = `
        INSERT INTO history_regional2z7w (${keys.join(', ')})
        VALUES (${placeholders})
        RETURNING *
        `;
    ...
    getKontrakHistory: async (userId) => {
        try {
            ...
            const historySql = `
            SELECT * FROM history_regional2z7w
            WHERE user_id = $1
    ...
    try {
        ...
        // Check if history_regional2z7w table exists before attempting to delete history records
        ...
        const historySql = `DELETE FROM history_regional2z7w WHERE user_id = $1`;
        await pool.query(historySql, [id]);
        }
    
        // Then delete the user
        const { sql, values } = Regional2Z7W._createDeleteQuery({ id });
        const result = await pool.query(sql, values);
        return result.rows[0];
    } catch (error) {
    ...
    try {
        ...
        const columns = [
        'id', 'nama_lengkap', 'jabatan', 'sebab_na', 'tampat_lahir', 'tanggal_lahir',
        'no_kontrak', 'kontrak_awal', 'kontrak_akhir',
        'tanggal_lahir', 'alamat_domisili', 'no_telepon', 'email',
        ...
        'pkwt_filename', 'pkwt_filepath', 'pkwt_mimetype', 'pkwt_filesize',
        // New document columns for regional2Z7W
        'no_ktp_filename', 'no_ktp_filepath', 'no_ktp_mimetype', 'no_ktp_filesize',
        'no_kk_filename', 'no_kk_filepath', 'no_kk_mimetype', 'no_kk_filesize',
        'no_npwp_filename', 'no_npwp_filepath', 'no_npwp_mimetype', 'no_npwp_filesize',
        'bpjs_kesehatan_karyawan_filename', 'bpjs_kesehatan_karyawan_filepath', 'bpjs_kesehatan_karyawan_mimetype', 'bpjs_kesehatan_karyawan_filesize',
        ...
        const sql = `
        SELECT ${columnsStr} FROM project_pertamina_ep_regional2zona7_wopdm
    ...
    try {
        ...
        'tanggal_lahir', 'no_telepon', 'kontak_darurat',
        'email', 'status_pernikahan', 'tempat_lahir',
        'jenis_kelamin', 'alamat_ktp', 'nama_ibu_kandung'
        ];

        const { sql, values } = Regional2Z7W._createSelectColumnsQuery(columns, { id });
        const result = await pool.query(sql, values);
        return result.rows[0];
    } catch (error) {
     ...
     * @param {number} userId - ID karyawan
     * @param {string} docType - Tipe dokumen (cv, ijazah, sertifikat, pkwt, no_ktp, no_kk, no_npwp, bpjs_kesehatan_karyawan, bpjstk, no_rekening)
     */
    ...
    updateUserWithHistory: async (id, userData, modifiedBy = 'system') => {
        try {
            ...
            const isContractChanged = Regional2Z7W._isContractChanged(oldUserData, userData);
            
            // Begin transaction
            await pool.query('BEGIN');
            
            // Update user data
            const updateResult = await Regional2Z7W.updateUser(id, userData);
  ...
  getHSEData: async () => {
    try {
      ...
      if (!tableExists) {
        console.error("Tabel project_pertamina_ep_regional2zona7_wopdm tidak ditemukan!");
        return [];
      }
      
      // Ambil semua data karyawan dengan kolom-kolom yang relevan untuk HSE
      const sql = `
        SELECT id, nama_lengkap, jabatan, sebab_na, nik_vendor, no_ktp, 
          tanggal_lahir, awal_mcu, akhir_mcu, hasil_mcu, vendor_mcu,
          alamat_domisili, no_telepon, no_hsepassport, awal_hsepassport, akhir_hsepassport,
          no_siml, awal_siml, akhir_siml
        FROM project_pertamina_ep_regional2zona7_wopdm
      ...
      if (error.code === '42P01') {
        console.error("Tabel project_pertamina_ep_regional2zona7_wopdm tidak ada");
        return [];
      }
      throw error;
    }
    ...
    try {
      const sql = `
        SELECT EXISTS (
          SELECT FROM information_schema.tables 
          WHERE table_schema = 'public' 
          AND table_name = 'regional2z7w_sertifikat'
        );
      `;
      const result = await pool.query(sql);
      return result.rows[0].exists;
  ...
  createSertifikatTable: async () => {
    try {
      ...
      const tableExists = await Regional2Z7W.checkSertifikatTableExists();
      
      if (tableExists) {
        console.log("Table regional2z7w_sertifikat already exists");
        return true;
      }
      
      // Create table if it doesn't exist
      const sql = `
        CREATE TABLE regional2z7w_sertifikat (
          id SERIAL PRIMARY KEY,
          user_id INTEGER NOT NULL REFERENCES project_pertamina_ep_regional2zona7_wopdm(id) ON DELETE CASCADE,
        ...
        CREATE INDEX IF NOT EXISTS idx_regional2z7w_sertifikat_user_id ON regional2z7w_sertifikat(user_id);
      `;
      
      await pool.query(sql);
      console.log("Table regional2z7w_sertifikat created successfully");
      return true;
    } catch (error) {
  ...
  addSertifikat: async (sertifikatData) => {
    try {
      ...
      const sql = `
        INSERT INTO regional2z7w_sertifikat (
  ...
  getSertifikatByUserId: async (userId) => {
    try {
      ...
      const sql = `
        SELECT * FROM regional2z7w_sertifikat
        WHERE user_id = $1
  ...
  updateSertifikat: async (id, sertifikatData) => {
    try {
      ...
      const sql = `
        UPDATE regional2z7w_sertifikat
        SET 
          judul_sertifikat = $1,
  ...
  deleteSertifikat: async (id) => {
    try {
      ...
      const sql = `
        DELETE FROM regional2z7w_sertifikat
        WHERE id = $1
        RETURNING *
      `;
