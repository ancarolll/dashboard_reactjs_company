...
class ManagementService {
    ...
    static async getAllDocuments(page = 1, perPage = 20, searchTerm = '', sectionFilter = 'all') {
        try {
            ...
            if (response.data && response.data.data && response.data.data.data) {
                ...
                console.log('Sample document structure:', Object.keys(firstDocument));
                console.log('Date fields:', {
                    uploadDate: firstDocument.upload_date,
                    awalBerlaku: firstDocument.awal_berlaku,
                    akhirBerlaku: firstDocument.akhir_berlaku
                });
            }
            ...
            // Tambahkan detail error yang lebih lengkap
    ...
    static async addDocument(documentData) {
        try {
            ...
            // Tambahkan data dokumen ke FormData
    ...
    static async addSection(name) {
        try {
        const response = await api.post(`${this.BASE_URL}/sections`, { name });
        return response.data;
        } catch (error) {
    ...
    // Utility function untuk memformat tanggal ke format DD/MM/YYYY
