...
const Pertamina2XViewProjectPage = () => {
    ...
    const downloadDocument = async (id, field) => {
        try {
            ...
            const response = await axios({
                url: `${API_BASE_URL}/regional2x/users/${id}/download/${field}`,
                method: 'GET',
                responseType: 'blob',
            ...
            if (response.data) {
                ...
                if (processedData.length > 0) {
                    ...
                    const usersWithFiles = processedData.filter(user => 
                        ...
                        user.bpjs_kesehatan_filename || 
                        user.bpjstk_filename || 
                        user.no_rekening_filename
                    );
    ...
    // DataTable columns definition - READ ONLY VERSION
    const columns = [
        { 
            name: 'Status', 
            cell: row => {
