...
const api = axios.create({
  baseURL: 'http://localhost:3005/api/regional2Z7D'
});
...
const TambahUserPertamina2Z7D = () => {
  const [formData, setFormData] = useState({
    nama_lengkap: '', no_kontrak: '', jabatan: '', tempat_lahir: '', tanggal_lahir: '', jenis_kelamin: '', no_telpon: '', 
    nama_ibu_kandung: '', kontak_darurat: '', nama_kontak_darurat: '', email: '', email_lain: '',alamat_ktp: '', 
    alamat_domisili: '', pendidikan: '', no_kk: '', no_ktp: '', no_npwp: '', no_rekening_mandiri: '', bpjs_kesehatan_karyawan: '', 
    bpjs_kesehatan_suami_istri: '', bpjs_anak1: '', bpjs_anak2: '', bpjs_anak3: '', bpjstk: '', bpjstk_keterangan: '', asuransi_lainnya: '',
    kontrak_awal: '', kontrak_akhir: '', cv: ''
  });

  const [showSertifikatModal, setShowSertifikatModal] = useState(false);
  
  // State untuk file uploads - Update untuk include file baru
  const [files, setFiles] = useState({
    ...
    bpjstk: null,
    no_rekening_mandiri: null
  });

  // State untuk file names - Update untuk include file baru
  ...
  // State untuk existing file info - Update untuk include file baru
  ...
  // State untuk deleted files - Update untuk include file baru
  ...
  // Tambahkan ref untuk file input baru
  ...
  const validateInputField = (name, value) => {
    ...
    if (name === 'bpjs_kesehatan_karyawan' && value) {
      if (value.length > 0 && value.length !== 13) {
        error = 'BPJS Health must be maximum 13 characters';
      }
    }
    ...
    if (name === 'no_kk' && value) {
      if (value.length > 0 && value.length !== 16) {
        error = 'Family Card Number must be maximum 16 digits';
      } else if (value.length > 0 && !/^\d+$/.test(value)) {
      ...
      const fetchFileInfo = async () => {
        try {
          ...
          if (response.data) {
            ...
            // File uploads baru yang ditambahkan
            ...
            if (userData.no_rekening_mandiri_filename) {
              setExistingFiles(prev => ({
                ...prev,
                no_rekening_mandiri: {
                  filename: userData.no_rekening_mandiri_filename,
                  filepath: userData.no_rekening_mandiri_filepath,
                  mimetype: userData.no_rekening_mandiri_mimetype,
                  filesize: userData.no_rekening_mandiri_filesize
                }
              }));
            }
          }
  ...
  const handleFileChange = (e, type) => {
    ...
    if (file) {
      ...
      if (!allowedTypes.includes(file.type)) {
        setErrors(prev => ({ ...prev, [type]: 'File type not supported' }));
        return;
      }
  ...
  const validateForm = () => {
    try {
      ...
      // Validasi kontrak akhir harus setelah kontrak awal - DIPERBARUI: TIDAK PERLU CEK MASA DEPAN
      ...
      try {
        ...
        dateFields.forEach(field => {
          if (dataToSubmit[field]) {
            // Ubah semua format ke YYYY-MM-DD
        ...
        if (isEditMode && userId) {
          ...
          setFormData({
            nama_lengkap: '', no_kontrak: '', jabatan: '', tempat_lahir: '', tanggal_lahir: '', jenis_kelamin: '', no_telpon: '', nama_ibu_kandung: '',
            kontak_darurat: '', nama_kontak_darurat: '', email: '', email_lain: '', alamat_ktp: '', alamat_domisili: '', pendidikan: '', no_kk: '',
            no_ktp: '', no_npwp: '', no_rekening_mandiri: '', bpjs_kesehatan_karyawan: '', bpjs_kesehatan_suami_istri: '', bpjs_anak1: '', bpjs_anak2: '', 
            bpjs_anak3: '', bpjstk: '', bpjstk_keterangan: '', kontrak_awal: '', kontrak_akhir: ''
          });          
          ...
          if (fileInputRefBPJSKes.current) fileInputRefBPJSKes.current.value = '';
  ...
  const uploadFiles = async (userId) => {
    // Tambahkan validasi userId
    if (!userId || userId === 'undefined' || userId === undefined) {
      throw new Error('Invalid User ID for file upload');
    }

    const fileTypes = ['cv', 'ijazah', 'sertifikat', 'pkwt', 'no_ktp', 'no_kk', 'no_npwp', 'bpjs_kesehatan_karyawan', 'bpjstk', 'no_rekening_mandiri'];
    ...
    if (uploadPromises.length > 0) {
      ...
      const errors = results.filter(result => result.error === true);
      if (errors.length > 0) {
        // Throw an error but don't break the whole submission process
      ...
      if (userId) {
        ...
        if (response.data && response.data.success) {
          ...
          const fileInputRefs = {
            ...
            bpjs_kesehatan_karyawan: fileInputRefBPJSKes,
            bpjstk: fileInputRefBPJSTK,
            no_rekening_mandiri: fileInputRefRekening
          };
    ...
    return (
      <div className="bg-white p-4 rounded-lg shadow mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          {/* Email Lain */}
          <div>
            <label className="block text-gray-700 text-sm font-bold mb-2" htmlFor="email_lain">
              Alternate Email
            </label>
            <input
              className={`shadow appearance-none border ${errors.email_lain ? 'border-red-500' : 'border-gray-300'} rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline`}
              type="email"
              name="email_lain"
              placeholder="email_lain@example.com"
              value={formData.email_lain || ''}
              onChange={handleChange}
            />
            {errors.email_lain && <p className="text-red-500 text-xs italic mt-1">{errors.email_lain}</p>}
          </div>
    ...
    return (
      <div className="bg-white p-4 rounded-lg shadow mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {/* Pendidikan */}
          <div>
            <label className="block text-gray-700 text-sm font-bold mb-2" htmlFor="pendidikan">
              Education
            </label>
            <input
              className={`shadow appearance-none border ${errors.pendidikan ? 'border-red-500' : 'border-gray-300'} rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline`}
              type="text"
              name="pendidikan"
              ...
              onChange={handleChange}
            />
            {errors.pendidikan && <p className="text-red-500 text-xs italic mt-1">{errors.pendidikan}</p>}
          </div>
          ...
          {/* Certificate Management Button - hanya tampil jika dalam mode edit dan userId tersedia */}
          {userId && (
            <div>
  ...
  // Bagian untuk menampilkan form data administrasi (KTP, KK, NPWP, rekening)
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <div className="flex flex-col">
              ...
              <small className="text-gray-500 mt-1">Maximum 16 digits.</small>
          ...
          <div>
            ...
            <div className="flex flex-col">
              ...
              <small className="text-gray-500 mt-1">15-16 digits.</small>
          ...
          {/* No Rekening Mandiri dengan upload */}
          <div>
            <label className="block text-gray-700 text-sm font-bold mb-2" htmlFor="no_rekening_mandiri">
              Mandiri Account Number
            </label>
            <div className="flex flex-col">
              <div className="flex">
                <input
                  className={`shadow appearance-none border ${errors.no_rekening_mandiri ? 'border-red-500' : 'border-gray-300'} rounded-l-md w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline`}
                  type="text"
                  name="no_rekening_mandiri"
                  placeholder="Enter Mandiri Account Number"
                  value={formData.no_rekening_mandiri || ''}
                  onChange={handleNumberChange}
                />
                <label className="flex items-center justify-center px-4 border border-l-0 border-gray-300 rounded-r-md bg-gray-50 cursor-pointer">
                  <FontAwesomeIcon icon={faUpload} className="text-gray-500" />
                  <input
                    ...
                    type="file"
                    name="no_rekening_mandiri"
                    onChange={(e) => handleFileChange(e, 'no_rekening_mandiri')}
                    className="hidden"
                    accept=".pdf,.doc,.docx,.jpg,.jpeg,.png"
                  />
                </label>
              </div>
              {errors.no_rekening_mandiri && <p className="text-red-500 text-xs italic mt-1">{errors.no_rekening_mandiri}</p>}
              {fileNames.no_rekening_mandiri && <p className="text-green-600 text-xs mt-1">Selected file: {fileNames.no_rekening_mandiri}</p>}
              {existingFiles.no_rekening_mandiri && !files.no_rekening_mandiri && (
                <div className="flex items-center mt-2">
                  <p className="text-blue-600 text-xs">Current file: {existingFiles.no_rekening_mandiri.filename}</p>
                  <div className="ml-2 flex">
                    <button
                      type="button"
                      className="text-blue-500 text-xs hover:text-blue-700 mr-2"
                      onClick={() => downloadFile('no_rekening_mandiri')}
                    >
                      Download
                    </button>
                    <button
                      type="button" 
                      className="text-red-500 text-xs hover:text-red-700"
                      onClick={() => deleteFile('no_rekening_mandiri')}
                    >
                      Delete
                    </button>
                  </div>
   ...
   const renderBPJSDataSection = () => {
    return (
      <div className="bg-white p-4 rounded-lg shadow mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {/* BPJS Kesehatan Karyawan dengan upload */}
          <div>
          ...
          <div>
            ...
            <input
              ...
              type="text"
              name="bpjs_anak1"
              placeholder="Enter Child 1 Insurance"
          ...
          <div>
            ...
            <input
              ...
              type="text"
              name="bpjs_anak2"
              placeholder="Enter Child 2 Insurance"
          ...
          <div>
            ...
            <input
              ...
              type="text"
              name="bpjs_anak3"
              placeholder="Enter Child 3 Insurance"
          ...
          {/* BPJS Ketenagakerjaan dengan upload */}
          <div>
          ...
          <div className="md:col-span-3">
            ...
            <textarea
              ...
              rows="2"
              placeholder="Enter BPJS Employment Info"
        ...
        <div className="flex-1 flex flex-col h-screen ml-68 bg-(--background-tar-color) overflow-y-auto px-6 py-3">
            ...
            <div className='flex flex-col bg-white p-6 shadow-md rounded gap-4'>
              <div>
                <h2 className="text-2xl font-bold mb-6 text-center text-gray-800">
                  {isEditMode 
                    ? isRestoreMode 
                      ? 'Form to Restore PT Pertamina EP Regional 2 Zone 7D Development Plan User Data' 
                      : 'Form to Edit PT Pertamina EP Regional 2 Zone 7D Development Plan User Data'
                    : 'Form to Add PT Pertamina EP Regional 2 Zone 7D Development Plan User Data'
                  }
                </h2>
                ...
                {submitted ? (
                  <div className="bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded relative mb-4">
                    ...
                    <p className="block sm:inline"> Form has been successfullyl {isEditMode ? 'diperbarui' : 'dikirim'}. Redirecting to user list page...</p>
                  </div>
                ) : (
                  <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="flex flex-row gap-x-10 items-center justify-center">
                      ...
                      <button 
                        ...
                        onClick={() => navigate('/regional2z7d-edit')}
                      >
                        Back
                      </button>
                    </div>
          ...
          {showSertifikatModal && (
            <SertifikatModal
              ...
              viewOnly={false}
              apiPrefix="regional2Z7D" // Sesuaikan dengan endpoint API
            />
          )}
...
export default TambahUserPertamina2Z7D