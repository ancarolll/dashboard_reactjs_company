...
import { faHouse, faFileContract, faHelmetSafety, faArrowRightFromBracket, faBell, faSun,faImage,faMagnifyingGlass, faCalendarAlt, faIdCard, faCarAlt,faLink
...
const DElnusa = () => {
  ...
  const getFilteredData = (data, searchTerm, filterType, dateField) => {
    ...
    if (filterType === 'all') {
      // If there's a search, filter based on search
      if (searchTerm) {
    ...
    let statusFiltered = data.filter(employee => {
      ...
      if (!employee[dateField]) {
        // Specifically for 'nonactive', also show those without a date
    ...
    // Filter based on search if there is one
  ...
  const getHSECounts = () => {
    ...
    // Total all employees, including those without HSE Passport date
    counts.total = backendData.length;

    // Count number without HSE
  ...
  const getSIMLCounts = () => {
    ...
    // Total all employees, including those without SIM date
    counts.total = backendData.length;

    // Count number without SIM
  ...
  const customStyles = {
    ...
    rows: { 
      style: { 
        minHeight: '60px',
        width: '100%' // Make sure rows take up the full available width
      },
    ...
    pagination: {
      style: {
        borderTopStyle: 'none', // Remove the top border on pagination
        width: '100%',
      },
    }
  };
