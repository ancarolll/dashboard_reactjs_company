// regional2s.controller.js - Updated version with document management

import { Regional2S } from "../../models/perusahaan/regional2s.model.js";
import fs from 'fs';
import path from "path";
import { Regional2sContractHistory } from "../../models/perusahaan/regional2s_contract_history.model.js";
...
export const Regional2sController = {
    /**
    ...
    index: async (req, res) => {
        try {
            ...
            if (req.query.format === 'json') {
                const dataKaryawan = await Regional2S.getAll();
                return res.json(dataKaryawan);
            }
            
            res.redirect('/api/regional2s/edit');
        } catch (error) {
    ...
    checkDbStructure: async (req, res) => {
        try {
            const tableStructure = await Regional2S.checkTableStructure();
            res.json({
    ...
    getEditPage: async (req, res) => {
        try {
            ...
            res.render('Regional2sEditProjectPage', {
                title: 'Edit Project Regional 2 Subsurface',
    ...
    getViewPage: async (req, res) => {
        try {
            ...
            const dataKaryawan = await Regional2S.getViewData();
            ...
            res.render('Regional2sViewProjectPage', {
                title: 'View Project Regional 2 Subsurface',
                karyawan: dataKaryawan,
                debugMode: process.env.NODE_ENV !== 'production'
            });
            
            console.log('Regional2sViewProjectPage rendered successfully');
        } catch (error) {
    ...
    getNaPage: async (req, res) => {
        try {
            ...
            const dataKaryawan = await Regional2S.getNAUsers();
            ...
            res.render('Regional2sNAProjectPage', {
                title: 'NA Project Regional 2 Subsurface',
                karyawan: dataKaryawan,
                debugMode: process.env.NODE_ENV !== 'production'
            });
        } catch (error) {
    ...
    getTambahUserForm: async (req, res) => {
        try {
            ...
            res.render('TambahUserRegional2s', {
                title: 'Tambah User Regional 2 Subsurface',
                karyawan: {},
                debugMode: process.env.NODE_ENV !== 'production'
            });
            
            console.log('TambahUserRegional2s form rendered successfully');
        } catch (error) {
    ...
    getEditUserForm: async (req, res) => {
        try {
            ...
            const dataKaryawan = await Regional2S.getUserById(id);
            ...
            res.render('TambahUserRegional2s', {
                title: 'Edit User Regional 2 Subsurface',
                karyawan: dataKaryawan,
                debugMode: process.env.NODE_ENV !== 'production'
            });
    ...
    addUser: async (req, res) => {
        try {
            ...
            const result = await Regional2S.addUser(userData);
            ...
            if (req.xhr || req.headers.accept.includes('application/json')) {
                return res.status(201).json({
                    ...
                    id: result.id, // Tambahkan ID secara eksplisit
                    data: result
                });
            }
            ...
            if (result.sebab_na) {
                res.redirect('/api/regional2s/na');
            } else {
                res.redirect('/api/regional2s/edit');
            }
        } catch (error) {
    ...
    updateUser: async (req, res) => {
        try {
            ...
            const oldUserData = await Regional2S.getUserById(userId);
            if (!oldUserData) {
            ...
            const result = await Regional2S.updateUserWithHistory(userId, userData, modifiedBy);
    ...
    setUserInactive: async (req, res) => {
        try {
            ...
            const userExists = await Regional2S.getUserById(id);
            if (!userExists) {
                return res.status(404).json({ 
                    success: false, 
                    message: 'Karyawan tidak ditemukan' 
            ...
            try {
                const result = await Regional2S.updateUserStatus(id, sebab_na);
    ...
    restoreUser: async (req, res) => {
        try {
            ...
            const userData = await Regional2S.getUserById(id);
            if (!userData) {
            ...
            const restoredUser = await Regional2S.restoreUser(id);
            ...
            res.redirect(`/api/regional2s/edit-user/${id}?restored=true&message=Perbarui tanggal kontrak dan nomor kontrak`);
        } catch (error) {
    ...
    deleteUser: async (req, res) => {
        try {
            ...
            const result = await Regional2S.deleteUser(id);
    ...
    /**
     * Upload dokumen (CV, Ijazah, Sertifikat, PKWT, KTP, KK, NPWP, BPJS, Rekening)
     * @param {object} req - Express request object
     * @param {object} res - Express response object
     */
    uploadDocument: async (req, res) => {
        try {
            ...
            // PERBAIKAN: Validasi docType dengan fungsi yang sudah ada
            if (!docType || !validateDocType(docType)) {
            ...
            const userExists = await Regional2S.getUserById(userIdInt);
            if (!userExists) {
            ...
            try {
            const result = await Regional2S.uploadDocument(userIdInt, docType, req.file);
            ...
            console.error(`[ERROR] Error umum saat upload:`, error);
    ...
    getDocumentFile: async (req, res) => {
        try {
            ...
            // Get employee data
            const employee = await Regional2S.getUserById(userId);
            if (!employee) {
            ...
            const possiblePaths = [
                ...
                path.join(__dirname, '../../uploads/regional2s', path.basename(normalizedPath))
            ];
    ...
    /**
     * Download dokumen (CV, Ijazah, Sertifikat, dll)
     * @param {object} req - Express request object
     * @param {object} res - Express response object
     */
    downloadDocument: async (req, res) => {
        try {
            ...
            const fileInfo = await Regional2S.getDocumentInfo(userId, docType);
    ...
    checkExpiredContracts: async (req, res) => {
        try {
            ...
            let expiredUsers = [];
            try {
                expiredUsers = await Regional2S.checkExpiredKontrak();
            } catch (checkError) {
            ...
            for (const user of expiredUsers) {
                try {
                    const updatedUser = await Regional2S.updateUserStatus(user.id, 'EOC');
    ...
    updateUserWithHistory: async (req, res) => {
        try {
            ...
            const updatedUser = await Regional2S.updateUser(id, userData);
            
            // Cek apakah contract history perlu diperbarui
            const oldUser = await Regional2S.getUserById(id);
            const contractChanged = 
            ...
            if (contractChanged) {
                try {
                    // Periksa apakah model Regional2sContractHistory ada
                    if (typeof Regional2sContractHistory !== 'undefined') {
                        ...
                        console.warn('Regional2sContractHistory not defined, skipping history recording');
                        // Gunakan alternatif Regional2S.saveKontrakHistory jika ada
                        if (typeof Regional2S.saveKontrakHistory === 'function') {
                            await Regional2S.saveKontrakHistory(id, oldUser, userData);
                        }
                    }
                } catch (historyError) {
    ...
    getContractChangeStats: async (req, res) => {
        try {
            const stats = await Regional2sContractHistory.getContractChangeStats();
    ...
    getKontrakHistory: async (req, res) => {
        try {
            ...
            // Panggil method getKontrakHistory dari model Regional2s
            const history = await Regional2S.getKontrakHistory(userId);
    ...
    getPersonalData: async (req, res) => {
        try {
            const id = req.params.id;
            const personalData = await Regional2S.getPersonalData(id);
    ...
    deleteDocument: async (req, res) => {
        try {
            ...
            if (!fileInfo || !fileInfo.filepath) {
                return res.status(404).json({ 
                    success: false,
                    message: `No ${docType} file needs to be deleted`,
                    warning: true 
                });
            }
            ...
            if (fileInfo.filepath && fs.existsSync(fileInfo.filepath)) {
                try {
                    ...
                    console.log(`File ${docType} deleted successfully: ${fileInfo.filepath}`);
                } catch (fsError) {
                    console.error(`Failed to delete physical file: ${fsError.message}`);
                    // Continue process even if file deletion fails
                }
            } else {
                console.log(`Physical file not found: ${fileInfo.filepath}`);
            }
            
            // Delete file reference from database
            await Regional2S.deleteDocument(userId, docType);
            
            res.json({ 
                success: true, 
                message: `${docType} file deleted successfully` 
            });
        } catch (error) {
    ...
    getCSVTemplate: async (req, res) => {
        try {
            // Definisi header CSV (disesuaikan dengan skema regional2s)
            const headers = [
                "nama_lengkap", "jabatan", "nik_vendor", "paket_tender", "no_kontrak", 
                "kontrak_awal", "kontrak_akhir", "tanggal_lahir", "alamat_domisili", "no_telpon", 
                "kontak_darurat", "nama_ibu_kandung", "gaji_net",
                "no_ktp", "no_kk", "jenis_kelamin", "status_pernikahan", "alamat_ktp", 
                "bpjs_kesehatan", "bpjstk", "bpjs_kesehatan_suami_istri", "bpjs_anak1", 
                "bpjs_anak2", "bpjs_anak3",
                "no_npwp", "email", "nama_institute_pendidikan", "jurusan", 
                "no_rekening", "nama_rekening", "nama_bank",
                "awal_mcu", "akhir_mcu", "hasil_mcu", "vendor_mcu", "no_hsepassport",
                "awal_hsepassport", "akhir_hsepassport", "no_siml", "awal_siml", "akhir_siml", "asuransi_lainnya", "cv"
            ];
            
            // Buat contoh data (satu baris contoh)
            const exampleData = [
                "John Doe", "Staff", "V12345", "Paket A", "KTR/2023/001",
                "12/11/2023", "31/12/2025", "10/11/1990", "Jl. Contoh No. 123", "08123456789",
                "08123456788", "Jane Doe", "5000000",
                "1234567890123456", "1234567890", "Laki-laki", "Menikah", "Jl. KTP No. 123",
                "12345678", "87654321", "12345679", "12345680", "12345681", "12345682",
                "12.345.678.9-012.345", "john.doe@example.com", "Universitas Contoh", "Teknik", 
                "1234567890", "John Doe", "Bank ABC",
                "12/11/2023", "31/12/2023", "Sehat", "RS Contoh", "HSE123456",
                "21/11/2023", "31/12/2023", "SIM123456", "12/11/2023", "31/12/2025", "123456", "done"
            ];
            ...
            res.setHeader('Content-Disposition', 'attachment; filename=template_data_karyawan_regional2s.csv');
    ...
    uploadBulkData: async (req, res) => {
        try {
            ...
            const validation = Regional2S.validateCSVData(parsedData);
            ...
            const processResult = await Regional2S.processBulkInsert(parsedData);
    ...
    /**
     * Mendapatkan data administrasi karyawan
     * @param {object} req - Express request object
     * @param {object} res - Express response object
     */
    getAdminData: async (req, res) => {
        try {
            const id = req.params.id;
            const adminData = await Regional2S.getAdminData(id);
    ...
    getCertificationData: async (req, res) => {
        try {
            const id = req.params.id;
            const certificationData = await Regional2S.getCertificationData(id);
    ...
    getHSEData: async (req, res) => {
        try {
            ...
            const dataHSE = await Regional2S.getHSEData();
    ...
    createSertifikatTable: async (req, res) => {
        try {
            await Regional2S.createSertifikatTable();
    ...
    addSertifikat: async (req, res) => {
        try {
            ...
            const result = await Regional2S.addSertifikat(sertifikatData);
    ...
    getSertifikatByUserId: async (req, res) => {
        try {
            ...
            // Dapatkan sertifikat
            const result = await Regional2S.getSertifikatByUserId(userId);
    ...
    getSertifikatById: async (req, res) => {
        try {
            ...
            // Dapatkan sertifikat
            const result = await Regional2S.getSertifikatById(id);
    ...
    updateSertifikat: async (req, res) => {
        try {
            ...
            // Update sertifikat
            const result = await Regional2S.updateSertifikat(id, sertifikatData);
    ...
    deleteSertifikat: async (req, res) => {
        try {
            ...
            // Hapus sertifikat
            const result = await Regional2S.deleteSertifikat(id);
