...
const Pertamina2Z7DNAProjectPage = () => {
  // State untuk data
  ...
  const fetchNonActiveData = async () => {
    ...
    try {
      try {
        const checkResult = await axios.get(`${API_BASE_URL}/${PROJECT_ID}/check-expired`, {
          headers: {
      ...
      const response = await axios.get(`${API_BASE_URL}/${PROJECT_ID}/na`, {
        headers: {
          'Accept': 'application/json',
      ...
      if (expiredWithoutSebabNA.length > 0) {
        ...
        for (const employee of expiredWithoutSebabNA) {
          try {
            await axios.put(`${API_BASE_URL}/${PROJECT_ID}/users/${employee.id}/na`, 
              { sebab_na: 'EOC' },
              {
                headers: {
                  'Content-Type': 'application/json',
            ...
            console.error(`Failed to update status for ${employee.nama_lengkap}:`, err);
          }
        }
        
        // Refresh data setelah update selesai
        if (expiredWithoutSebabNA.length > 0) {
          const refreshResponse = await axios.get(`${API_BASE_URL}/${PROJECT_ID}/na`, {
            headers: {
              'Accept': 'application/json'
            },
            params: {
      ...
      try {
        const backupResponse = await axios.get(`${API_BASE_URL}/${PROJECT_ID}/users`, {
          headers: {
            'Accept': 'application/json'
          }
        });
  ...
  const manualCheckExpired = async () => {
    try {
      ...
      const response = await axios.get(`${API_BASE_URL}/${PROJECT_ID}/users`, {
      ...
      const updatePromises = expiredNoSebabNA.map(user => 
        axios.put(`${API_BASE_URL}/${PROJECT_ID}/users/${user.id}/na`, 
          { sebab_na: 'EOC' },
          {
            headers: {
              'Content-Type': 'application/json',
  ...
  const deleteUserFromBackend = async (id) => {
    try {
      ...
      const response = await axios.delete(`${API_BASE_URL}/${PROJECT_ID}/users/${id}`, {
        headers: {
          'Accept': 'application/json'
        }
      });
  ...
  const restoreUserToActive = async (id) => {
    try {
      ...
      const response = await axios.put(
        `${API_BASE_URL}/${PROJECT_ID}/users/${id}/restore`, 
        {},
        {
          headers: {
            'Content-Type': 'application/json',
  ...
  const formatDate = (dateString) => {
    ...
    try {
      // Format tanggal DD/MM/YYYY yang konsisten
  ...
  const handleRestore = (user) => {
    ...
    const confirmMessage = `Are you sure you want to restore employee "${user.nama_lengkap}" to active status?

IMPORTANT: 
- This action will remove the inactive reason (${user.sebab_na || '-'})
- You can update the contract dates and contract number as needed
- Contract dates can be set to any date (past, present, or future)`;
    ...
    if (window.confirm(confirmMessage)) {
      ...
      try {
        ...
        axios.put(
          `${API_BASE_URL}/${PROJECT_ID}/users/${user.id}/restore`,
          {},
          {
            headers: {
              'Content-Type': 'application/json',
        ...
        .then(response => {
          ...
          // Persiapkan data untuk halaman edit
          ...
          setTimeout(() => {
            ...
            navigate('/tambah-user-regional2z7d', {
              state: {
                editMode: true,
                userData: userData,
  ...
  const handleDeleteAll = () => {
    ...
    if (window.confirm(`Are you sure you want to permanently delete all ${filteredRecords.length} data?`)) {
  ...
  const downloadFile = async (userId, docType, fileName) => {
    try {
      const response = await axios.get(
        `${API_BASE_URL}/${PROJECT_ID}/users/${userId}/download/${docType}`,
        { responseType: 'blob' }
      );
  ...
  const handleExportAllData = () => {
    ...
    exportToExcel(filteredRecords, 'all_inactive_employee_data_regional2Z7D');
  };
  ...
  const handleExportSelectedData = () => {
    ...
    exportToExcel(selectedRows, 'selected_inactive_employee_data_regional2Z7D');
  };
...
const formatWaktuSA = (dateTimeString) => {
  ...
  const columns = [
    ...
    { name: 'Full Name',  selector: row => row.nama_lengkap, sortable: true, width: "auto", style: { minWidth: "250px" } },
    ...
    { name: 'Contract Number', selector: row => row.no_kontrak, sortable: true, width: "auto", style: { minWidth: "250px" } },
    { name: 'Start Contract ', selector: row => row.kontrak_awal, sortable: true, cell: row => formatDate(row.kontrak_awal),width: "150px" },
    { name: 'End Contract ', selector: row => row.kontrak_akhir, sortable: true, cell: row => formatDate(row.kontrak_akhir), width: "150px" }
  ];
...
export default Pertamina2Z7DNAProjectPage