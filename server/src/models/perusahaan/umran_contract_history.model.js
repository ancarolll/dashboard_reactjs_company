...
export const UmranContractHistory = {
    /**
    ...
    addHistory: async (historyData) => {
        ...
        const query = `
            INSERT INTO umran_contract_history 
            (${keys.join(', ')})
            VALUES (${placeholders})
            RETURNING *
        `;
    ...
    getHistoryByUserId: async (userId) => {
        ...
        const checkTableQuery = `
            ...
            AND table_name = 'umran_contract_history'
            );
        `;
        ...
        if (!tableExists.rows[0].exists) {
            console.error("Table umran_contract_history does not exist");
            return [];
        }
        
        // Use parameterized query with better formatting
        const query = `
            SELECT * FROM umran_contract_history
            WHERE user_id = $1
        ...
        if (result.rowCount === 0) {
            const diagnosticQuery = `SELECT COUNT(*) FROM umran_contract_history`;
            ...
            const sampleQuery = `SELECT * FROM umran_contract_history LIMIT 1`;
    ...
    getAllContractHistory: async (req, res) => {
        ...
        const history = await UmranContractHistory.getAllHistoryByUserId(userId);
