...
const Pertamina4NAProjectPage = () => {
  ...
  const fetchNonActiveData = async () => {
    ...
    try {
      ...
      try {
        const checkResult = await axios.get(`${API_BASE_URL}/regional4/check-expired`, {
          headers: {
      ...
      const response = await axios.get(`${API_BASE_URL}/regional4/na`, {
        headers: {
          'Accept': 'application/json',
      ...
      if (expiredWithoutSebabNA.length > 0) {
        ...
        for (const employee of expiredWithoutSebabNA) {
          try {
            await axios.put(`${API_BASE_URL}/regional4/users/${employee.id}/na`, 
              { sebab_na: 'EOC' },
              {
                headers: {
                  'Content-Type': 'application/json',
        ...
        if (expiredWithoutSebabNA.length > 0) {
          const refreshResponse = await axios.get(`${API_BASE_URL}/regional4/na`, {
            headers: {
              'Accept': 'application/json'
            },
            params: {
      ...
      try {
        const backupResponse = await axios.get(`${API_BASE_URL}/regional4/users`, {
          headers: {
            'Accept': 'application/json'
          }
        });
  ...
  const manualCheckExpired = async () => {
    try {
      ...
      const updatePromises = expiredNoSebabNA.map(user => 
        axios.put(`${API_BASE_URL}/regional4/users/${user.id}/na`, 
          { sebab_na: 'EOC' },
          {
            headers: {
              'Content-Type': 'application/json',
  ...
  const deleteUserFromBackend = async (id) => {
    try {
      ...
      const response = await axios.delete(`${API_BASE_URL}/regional4/users/${id}`, {
        headers: {
          'Accept': 'application/json'
        }
      });
  ...
  const restoreUserToActive = async (id) => {
    try {
      ...
      const response = await axios.put(
        `${API_BASE_URL}/regional4/users/${id}/restore`, 
        {},
        {
          headers: {
            'Content-Type': 'application/json',
  ...
  const getStatusColor = (daysRemaining) => {
    ...
    if (daysRemaining <= 44) return 'bg-green-200';
    return '';
  };
...
- The contract end date must be in the future`;
    ...
    if (window.confirm(confirmMessage)) {
      ...
      try {
        ...
        axios.put(
          `${API_BASE_URL}/regional4/users/${user.id}/restore`,
          {},
          {
            headers: {
              'Content-Type': 'application/json',
        ...
        setMessage({text: "Restoration successful! Redirecting to edit page...", type: "success"});
          ...
          setTimeout(() => {
            ...
            navigate('/tambah-user-regional4', {
              state: {
                editMode: true,
                userData: userData,
                projectId: 'regional4',
  ...
  const handleDeleteSelected = () => {
    ...
    if (window.confirm(`Are you sure you want to permanently delete ${selectedRows.length} selected data?`)) {
  ...
  const downloadFile = async (userId, docType, fileName) => {
    try {
      const response = await axios.get(
        `${API_BASE_URL}/regional4/users/${userId}/download/${docType}`,
        { responseType: 'blob' }
      );
  ...
  const handleExportAllData = () => {
    ...
    exportToExcel(filteredRecords, 'all_inactive_employee_data');
  };
  ...
  const handleExportSelectedData = () => {
    ...
    exportToExcel(selectedRows, 'selected_inactive_employee_data');
  };
...
const formatWaktuSA = (dateTimeString) => {
  ...
  const columns = [
    ...
    {name: 'TAR ID', selector: row => row.nik_tar, sortable: true, width: "120px"},
    {name: 'PEP Reg4 ID', selector: row => row.nik_pep_reg4, sortable: true, width: "120px"},
    ...
    {name: 'Contract Start',  selector: row => row.kontrak_awal,  sortable: true,  cell: row => formatDate(row.kontrak_awal), width: "150px"  },
    {name: 'Contract End',  selector: row => row.kontrak_akhir,  sortable: true,  cell: row => formatDate(row.kontrak_akhir),  width: "150px"  }
  ];
...
export default Pertamina4NAProjectPage