import { Regional4 } from '../../models/perusahaan/regional4.model.js'
import fs from 'fs';
import path from 'path';
import { Regional4ContractHistory } from '../../models/perusahaan/regional4_contract_history.model.js';
...
export const Regional4Controller = {
    /**
   ...
   */
    ...
    index: async (req, res) => {
      try {
          ...
          if (req.query.format === 'json') {
              const dataKaryawan = await Regional4.getAll();
              return res.json(dataKaryawan);
          }
          
          res.redirect('/api/regional4/edit');
      } catch (error) {
    ...
    checkDbStructure: async (req, res) => {
        try {
            const tableStructure = await Regional4.checkTableStructure();
            res.json({
    ...
    getEditPage: async (req, res) => {
        try {
            ...
            res.render('Regional4EditProjectPage', {
                title: 'Edit Project Regional 4',
   ...
   */
    ...
    getViewPage: async (req, res) => {
        try {
            ...
            const dataKaryawan = await Regional4.getViewData();
            ...
            res.render('Regional4ViewProjectPage', {
                title: 'View Project Regional 4',
                karyawan: dataKaryawan,
                debugMode: process.env.NODE_ENV !== 'production'
            });
            
            console.log('Regional4ViewProjectPage rendered successfully');
        } catch (error) {
   ...
   */
    ...
    getNaPage: async (req, res) => {
        try {
            ...
            const dataKaryawan = await Regional4.getNAUsers();
            ...
            res.render('Regional4NAProjectPage', {
                title: 'NA Project Regional 4',
                karyawan: dataKaryawan,
                debugMode: process.env.NODE_ENV !== 'production'
            });
        } catch (error) {
   ...
   */
    ...
    getTambahUserForm: async (req, res) => {
        try {
            ...
            res.render('TambahUserRegional4', {
                title: 'Tambah User Regional 4',
                karyawan: {},
                debugMode: process.env.NODE_ENV !== 'production'
            });
            
            console.log('TambahUserRegional4 form rendered successfully');
        } catch (error) {
    ...
    getEditUserForm: async (req, res) => {
        try {
            ...
            const dataKaryawan = await Regional4.getUserById(id);
   ...
   */
    ...
    addUser: async (req, res) => {
        ...
        const result = await Regional4.addUser(userData);
   ...
   */
    ...
    updateUser: async (req, res) => {
        try {
          ...
          const oldUserData = await Regional4.getUserById(userId);
          if (!oldUserData) {
          ...
          const result = await Regional4.updateUserWithHistory(userId, userData, modifiedBy);
    ...
    setUserInactive: async (req, res) => {
        try {
          ...
          const userExists = await Regional4.getUserById(id);
          if (!userExists) {
            return res.status(404).json({ 
              success: false, 
              message: 'Karyawan tidak ditemukan' 
          ...
          try {
            const result = await Regional4.updateUserStatus(id, sebab_na);
     ...
     */
      ...
      restoreUser: async (req, res) => {
        try {
          ...
          const userData = await Regional4.getUserById(id);
          if (!userData) {
          ...
          const restoredUser = await Regional4.restoreUser(id);
          ...
          res.redirect(`/api/regional4/edit-user/${id}?restored=true&message=Perbarui tanggal kontrak dan nomor kontrak`);
        } catch (error) {
      ...
      deleteUser: async (req, res) => {
          try {
              ...
              const result = await Regional4.deleteUser(id);
    ...
    uploadDocument: async (req, res) => {
      try {
        ...
        const userExists = await Regional4.getUserById(userIdInt);
        if (!userExists) {
          return res.status(404).json({ 
        ...
        const result = await Regional4.uploadDocument(userId, docType, req.file);
    ...
    downloadDocument: async (req, res) => {
      try {
        ...
        const fileInfo = await Regional4.getDocumentInfo(userId, docType);
    ...
    getDocumentFile: async (req, res) => {
      try {
        ...
        // Get employee data
        const employee = await Regional4.getUserById(userId);
        if (!employee) {
        ...
        const possiblePaths = [
          ...
          path.join('D:/dashboardadmin/server/src/uploads/regional4-files', path.basename(normalizedPath))
        ];
   ...
   */
     checkExpiredContracts: async (req, res) => {
      try {
        ...
        let expiredUsers = [];
        try {
          expiredUsers = await Regional4.checkExpiredKontrak();
        } catch (checkError) {
      ...
      for (const user of expiredUsers) {
        try {
          const updatedUser = await Regional4.updateUserStatus(user.id, 'EOC');
   ...
   */
    ...
    updateUserWithHistory: async (req, res) => {
        try {
          ...
          const updatedUser = await Regional4.updateUser(id, userData);
          
          // Cek apakah contract history perlu diperbarui
          const oldUser = await Regional4.getUserById(id);
          const contractChanged = 
          ...
          if (contractChanged) {
            try {
              // Periksa apakah model Regional4ContractHistory ada
              if (typeof Regional4ContractHistory !== 'undefined') {
                ...
                console.warn('Regional4ContractHistory not defined, skipping history recording');
                // Gunakan alternatif Regional4.saveKontrakHistory jika ada
                if (typeof Regional4.saveKontrakHistory === 'function') {
                  await Regional4.saveKontrakHistory(id, oldUser, userData);
                }
              }
            } catch (historyError) {
  ...
  getAllContractHistory: async (req, res) => {
    try {
      ...
      const history = await Regional4ContractHistory.getAllHistoryByUserId(userId);
  ...
  getContractChangeStats: async (req, res) => {
    try {
      const stats = await Regional4ContractHistory.getContractChangeStats();
  ...
  getKontrakHistory: async (req, res) => {
    try {
      ...
      // Panggil method getKontrakHistory dari model Regional4
      const history = await Regional4.getKontrakHistory(userId);
  ...
  getPersonalData: async (req, res) => {
    try {
        const id = req.params.id;
        const personalData = await Regional4.getPersonalData(id);
  ...
  deleteDocument: async (req, res) => {
    try {
      ...
      await Regional4.deleteDocument(userId, docType);
  ...
  getCSVTemplate: async (req, res) => {
    try {
      // Definisi header CSV (disesuaikan dengan skema regional4)
      const headers = [
        "nama_karyawan", "jabatan", "nik_tar", "nik_pep_reg4", "no_kontrak", 
        "kontrak_awal", "kontrak_akhir", "tanggal_lahir", "alamat_domisili", "nomor_hp", 
        "nama_kontak_emergency", "no_telp_kontak_emergency", "hubungan_kontak_emergency",
        "nik", "no_kk", "nama_pasangan", "nama_anak_ke_1", "nama_anak_ke_2", 
        "status_nikah", "bpjs_kesehatan", "bpjs_ketenagakerjaan", "asuransi_lainnya",
        "npwp", "alamat_email", "pendidikan_terakhir", "universitas", "jurusan",
        "ukuran_sepatu", "ukuran_coveroll", "nomor_rekening", "nama_pemilik", "bank",
      ...
      const exampleData = [
        ...
        "12/01/2023", "31/12/2025", "10/11/1990", "Jl. Contoh No. 123", "08123456789",
        "Jane Doe", "08123456788", "Istri", "1234567890123456", "1234567890",
        "Jane Doe", "Kid One", "Kid Two", "Menikah", "12345678", "87654321", "87654321",
        "12.345.678.9-012.345", "john.doe@example.com", "S1", "Universitas Contoh", "Teknik", 
        "42", "L", "1234567890", "John Doe", "Bank ABC",
        "01/01/2023", "31/12/2023", "Sehat", "RS Contoh", "HSE123456",
        "01/01/2023", "31/12/2023", "SIM123456", "01/01/2023", "31/12/2025", "done"
      ];
      ...
      res.setHeader('Content-Disposition', 'attachment; filename=template_data_karyawan_regional4.csv');
  ...
  uploadBulkData: async (req, res) => {
    try {
      ...
      const validation = Regional4.validateCSVData(parsedData);
      ...
      const processResult = await Regional4.processBulkInsert(parsedData);
  ...
  getAdminData: async (req, res) => {
    try {
        const id = req.params.id;
        const adminData = await Regional4.getAdminData(id);
  ...
  getCertificationData: async (req, res) => {
    try {
        const id = req.params.id;
        const certificationData = await Regional4.getCertificationData(id);
  ...
  getHSEData: async (req, res) => {
    try {
      ...
      const dataHSE = await Regional4.getHSEData();
 ...
 */
  updateHSEDataWithHistory: async (req, res) => {
    try {
      ...
      const isHSEChanged = Regional4HSEHistory.isHSEDataChanged(oldUserData, hseUpdateData);
      ...
      try {
        ...
        const updatedUser = await Regional4.updateUser(id, hseUpdateData);
        ...
        if (isHSEChanged) {
          try {
            ...
            const historyData = Regional4HSEHistory.prepareHistoryData(oldUserData, hseUpdateData, modifiedBy);
            
            // Tambahkan catatan ke riwayat
            await Regional4HSEHistory.addHistory(historyData);
        ...
        console.log(`Berhasil memperbarui data HSE untuk karyawan: ${updatedUser.nama_karyawan}`);
  ...
  getHSEHistory: async (req, res) => {
    try {
      ...
      // Pastikan user ada
      const userExists = await Regional4.getUserById(userId);
      if (!userExists) {
        return res.status(404).json({
          success: false,
          message: 'Karyawan tidak ditemukan'
      ...
      const history = await Regional4HSEHistory.getAllHistoryByUserId(userId);
  ...
  getHSEHistoryStats: async (req, res) => {
    try {
      ...
      // Dapatkan statistik
      const stats = await Regional4HSEHistory.getHSEChangeStats();
  ...
  try {
    await Regional4.createSertifikatTable();
  ...
  try {
    ...
    // Add the certificate
    const result = await Regional4.addSertifikat(sertifikatData);
  ...
  try {
    ...
    // Get certificates
    const result = await Regional4.getSertifikatByUserId(userId);
  ...
  try {
    ...
    // Get the certificate
    const result = await Regional4.getSertifikatById(id);
  ...
  try {
    ...
    const result = await Regional4.updateSertifikat(id, sertifikatData);
  ...
  try {
    ...
    const result = await Regional4.deleteSertifikat(id);
