...
export class Reg2sBudgetController {
  // Get master budget and all absorptions
  static async getBudgetData(req, res) {
    try {
      // Get master budget
      const masterBudget = await Reg2sBudgetModel.getMasterBudget();
      ...
      const absorptions = await Reg2sBudgetModel.getAllAbsorptions(
        filters, 
        parseInt(page), 
        parseInt(limit)
      );
  ...
  static async createOrUpdateMaster(req, res) {
    try {
      ...
      // Create or update master budget
      const result = await Reg2sBudgetModel.createOrUpdateMaster(budgetData, username);
  ...
  static async addPeriodAbsorption(req, res) {
    try {
      ...
      // Add period absorption
      const result = await Reg2sBudgetModel.addPeriodAbsorption(absorptionData, username);
  ...
  static async deleteAbsorption(req, res) {
    try {
      ...
      // Delete absorption
      const result = await Reg2sBudgetModel.deleteAbsorption(parseInt(id), username);
  ...
  static async getBudgetHistory(req, res) {
    try {
      const history = await Reg2sBudgetModel.getBudgetHistory();
  ...
  static async getFilterOptions(req, res) {
    try {
      const options = await Reg2sBudgetModel.getFilterOptions();
  ...
  static async getBudgetSummary(req, res) {
    try {
      const summary = await Reg2sBudgetModel.getBudgetSummary();
  ...
  static async importExcel(req, res) {
    try {
      ...
      const result = await Reg2sBudgetModel.importFromExcel(mappedData, username);
...
export default Reg2sBudgetController;