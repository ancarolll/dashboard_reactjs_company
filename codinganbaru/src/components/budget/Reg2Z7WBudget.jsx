...
/**
 * Reg2Z7WBudget Component - Displays and manages Regional 2 Z7W budget data
 * 
...
const Reg2Z7WBudget = ({ 
  ...
  const showMasterForm = () => {
    ...
    if (budgetData.master) {
      masterForm.setFieldsValue({
        ...
        total_termin5: parseFloat(budgetData.master.total_termin5 || 0),
        total_termin6: parseFloat(budgetData.master.total_termin6 || 0),
  ...
  const showEditProjectForm = (project) => {
    ...
    projectForm.setFieldsValue({
      ...
      termin5: parseFloat(project.termin5 || 0),
      termin6: parseFloat(project.termin6 || 0),
  ...
  const calculateProjectTotals = () => {
    ...
    const termin5 = parseFloat(values.termin5 || 0);
    const termin6 = parseFloat(values.termin6 || 0);
  ...
  const handleExportExcel = async () => {
    try {
      ...
      let filename = 'regional2z7w_budget_export.xlsx';
  ...
  return (
    <div className="reg2x-budget-container" style={{ padding: embedded ? 0 : '20px' }}>
      {!embedded && (
        <Title level={2}>Regional 2 Z7W Budget Management</Title>
      )}
                  ...
                  <Row gutter={16}>
                    ...
                    <Col xs={24} sm={4}>
                      <Statistic
                        ...
                        value={budgetData.master ? parseFloat(budgetData.master.total_termin5 || 0) : 0}
                        precision={0}
                        formatter={(value) => `Rp ${formatNumber(value)}`}
                      />
                    </Col>
                    <Col xs={24} sm={4}>
                      <Statistic
                        ...
                        value={budgetData.master ? parseFloat(budgetData.master.total_termin6 || 0) : 0}
                        precision={0}
                        formatter={(value) => `Rp ${formatNumber(value)}`}
                      />
                    </Col>
        ...
        >
          ...
          <Row gutter={16}>
            ...
            <Col span={8}>
              <Form.Item
                name="total_termin5"
                label="Total Termin 5"
              >
                <InputNumber 
                  style={{ width: '100%' }}
                  formatter={value => `${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ',')}
                  parser={value => value.replace(/\$\s?|(,*)/g, '')}
                  placeholder="Enter total termin 5" 
                  min={0}
                  disabled={true}
                  title="This value is automatically calculated from projects"
                />
            ...
            <Col span={8}>
              <Form.Item
                name="total_termin6"
                label="Total Termin 6"
              >
                <InputNumber 
                  style={{ width: '100%' }}
                  formatter={value => `${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ',')}
                  parser={value => value.replace(/\$\s?|(,*)/g, '')}
                  placeholder="Enter total termin 6" 
                  min={0}
                  disabled={true}
                  title="This value is automatically calculated from projects"
                />
