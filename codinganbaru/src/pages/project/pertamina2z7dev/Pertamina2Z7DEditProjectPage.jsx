...
import Reg2Z7DBudget from '../../../components/budget/Reg2Z7DBudget';
...
const Pertamina2Z7DEditProjectPage = () => {
  ...
  const handleMassalClick = () => {
    navigate('/upload-massal-regional2z7d', { 
    state: { 
      bulkMode: true,
      projectId: PROJECT_ID
    } 
  ...
  const uploadFile = async (file, isReplace = false) => {
    ...
    const response = await axios.post(
      `${API_BASE_URL}/regional2Z7D/users/${fileUploadModal.userId}/upload/${fileUploadModal.docType}`,
      formData,
      {
      headers: {
  ...
  const deleteUserFromBackend = async (id) => {
    ...
    const response = await axios.delete(`${API_BASE_URL}/regional2Z7D/users/${id}`, {
      headers: {
      'Accept': 'application/json'
      }
    });
  ...
  const setUserInactive = async (id, reason) => {
    ...
    const response = await axios.put(`${API_BASE_URL}/regional2Z7D/users/${id}/na`, 
      { sebab_na: reason },
      {
      headers: {
        'Content-Type': 'application/json',
  ...
  const handleDeleteConfirm = (reason) => {
    ...
    axios.put(
    `${API_BASE_URL}/regional2Z7D/users/${userToDelete.id}/na`, 
    { 
      sebab_na: reason,
  ...
  const columns = [
    ...
    { name: 'BPJS Health',
      cell: row => (
        ...
        >
          {row.bpjs_kesehatan_karyawan || 'View BPJS Health'}
    ...
    { name: 'Employment Insurance Details', selector: row => row.bpjstk_keterangan || '', sortable: true, width: "600px" },
    ...
    { name: 'Mandiri Account Number',
      cell: row => (
        ...
        >
          {row.no_rekening_mandiri || 'View Account Number'}
    ...
    return (
      <div className='bg-(--background-tar-color)'> 
        ...
        <main className="flex-1">
          ...
          <StatBoxPertamina2Z7DComponents onFilterChange={handleFilterChange} activeFilter={activeFilter}/>
          </div>
        ...
        {loading ? (
          ...
          <ButtonComponents 
            variant='primary'
            onClick={() => navigate('/tambah-user-regional2z7d')}
          >
            Add New User
          </ButtonComponents>
          </div>
...
export default Pertamina2Z7DEditProjectPage