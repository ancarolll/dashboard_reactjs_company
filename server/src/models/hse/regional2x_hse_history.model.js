...
export const Regional2xHSEHistory = {
    /**
    ...
    ensureTableExists: async () => {
        ...
        const checkTableSql = `
            ...
            AND table_name = 'hse_regional2x_history'
            );
        `;
        ...
        const createTableSql = `
            CREATE TABLE hse_regional2x_history (
            ...
            FOREIGN KEY (user_id) REFERENCES project_pertamina_ep_regional2_eksplorasi(id) ON DELETE CASCADE
            );
        `;
    ...
    addHistory: async (historyData) => {
        try {
        // Ensure table exists
        await Regional2xHSEHistory.ensureTableExists();
        ...
        const sql = `
            INSERT INTO hse_regional2x_history (${keys.join(', ')})
            VALUES (${placeholders})
            RETURNING *
        `;
    ...
    getAllHistoryByUserId: async (userId) => {
        ...
        const sql = `
            SELECT * FROM hse_regional2x_history
            WHERE user_id = $1
