...
export class URPBudgetController {
  // Get master budget and all absorptions
  static async getBudgetData(req, res) {
    try {
      // Get master budget
      const masterBudget = await URPBudgetModel.getMasterBudget();
      ...
      const absorptions = await URPBudgetModel.getAllAbsorptions(
        filters, 
        parseInt(page), 
        parseInt(limit)
      );
  ...
  static async createOrUpdateMaster(req, res) {
    try {
      ...
      // Create or update master budget
      const result = await URPBudgetModel.createOrUpdateMaster(budgetData, username);
  ...
  static async addPeriodAbsorption(req, res) {
    try {
      ...
      // Add period absorption
      const result = await URPBudgetModel.addPeriodAbsorption(absorptionData, username);
  ...
  static async deleteAbsorption(req, res) {
    try {
      ...
      // Delete absorption
      const result = await URPBudgetModel.deleteAbsorption(parseInt(id), username);
  ...
  static async getBudgetHistory(req, res) {
    try {
      const history = await URPBudgetModel.getBudgetHistory();
  ...
  static async getFilterOptions(req, res) {
    try {
      const options = await URPBudgetModel.getFilterOptions();
  ...
  static async getBudgetSummary(req, res) {
    try {
      const summary = await URPBudgetModel.getBudgetSummary();
  ...
  static async importExcel(req, res) {
    try {
      ...
      const result = await URPBudgetModel.importFromExcel(mappedData, username);
  ...
  static async exportExcel(req, res) {
    try {
        ...
        // Create absorptions worksheet with fields from the urp_budget_absorption table only
        ...
        const fileName = `urp_budget_absorptions_export_${timestamp}.xlsx`;
...
export default URPBudgetController;