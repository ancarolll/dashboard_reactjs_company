...
import URPBudget from '../../../components/budget/URPBudget';
...
const UmranEditProjectPage = () => {
  ...
  // State for document modal (added from Pertamina4)
  ...
  // State for sertifikat modal (added from Pertamina4)
  ...
  // State for file preview modal (added from Pertamina4)
  ...
  // Sertifikat modal handlers (added from Pertamina4)
  ...
  // Document modal handlers (added from Pertamina4)
  ...
  // Close document modal (added from Pertamina4)
  ...
  // Function to preview a file (added from Pertamina4)
  ...
  // Function to download a document (added from Pertamina4)
  ...
  const fetchDataFromBackend = async () => {
    ...
    try {
      // console.log("Starting to fetch data from the API...");
      ...
      if (response.data) {
        if (Array.isArray(response.data)) {
          ...
          if (response.data.length > 0) {
            ...
            // console.log('Sample date format from API:', {
            //   kontrak_awal: sampleUser.kontrak_awal,
            //   kontrak_akhir: sampleUser.kontrak_akhir
            // });
          }
  ...
  const uploadFile = async (file, isReplace = false) => {
    ...
    if (!allowedMimeTypes.includes(selectedFile.type)) {
      setMessage({ 
        text: 'File format not supported. Use PDF, DOC, DOCX, JPG, or PNG.', 
        type: 'error' 
      });
      return;
    }
    ...
    try {
      const response = await axios.post(
        `${API_BASE_URL}/umran/users/${fileUploadModal.userId}/upload/${fileUploadModal.docType}`,
        formData,
        {
          headers: {
  ...
  const setUserInactive = async (id, reason) => {
    try {
      ...
      const response = await axios.put(`${API_BASE_URL}/umran/users/${id}/na`, 
        { sebab_na: reason },
        {
          headers: {
            'Content-Type': 'application/json',
  ...
  useEffect(() => {
    if (location.state) {
      if (location.state.addedUser && location.state.newUser) { 
        ...
        setMessage({ 
          text: `User data for ${newUserData.nama_lengkap_karyawan || 'new user'} has been successfully added!`,
          type: 'success' 
        });
        ...
        setMessage({ 
          text: `User data for ${updatedUserData.nama_lengkap_karyawan || 'edited user'} has been successfully updated!`, 
          type: 'success' 
        });
        window.history.replaceState({}, document.title);
        fetchDataFromBackend();
  ...
  const handleDeleteConfirm = (reason) => {
    ...
    setMessage({ text: `Processing inactive status for ${userToDelete.nama_lengkap_karyawan}...`, type: 'info' });
  ...
  axios.put(
    `${API_BASE_URL}/umran/users/${userToDelete.id}/na`, 
    { 
      sebab_na: reason,
  ...
  .then(response => {
    ...
    setMessage({ 
      text: `user ${userToDelete.nama_lengkap_karyawan} has been successfully moved to inactive status!`,
      type: 'success' 
    });
  ...
  const handleEdit = (user) => {
    ...
    // console.log('Original data before sending to edit page:', user);
  ...
  const handleExportAllData = () => {
    ...
    exportToExcel(backendData, 'all_employee_data_umran');
  };
  ...
  // DataTable columns definition sesuai dengan struktur tabel Umran
  const columns = [
    { 
      name: 'Action', 
      cell: row => (
    ...
    { name: 'Full Name', selector: row => row.nama_lengkap_karyawan || '', sortable: true, width: "350px" },
    ...
    { name: 'BPJS Health Status', selector: row => row.status_bpjs || '', sortable: true, width: "250px" },
    ...
    { name: 'Transportation Allowance', selector: row => row.t_transport || '', sortable: true, width: "250px", cell: row => row.t_transport ? `Rp ${parseFloat(row.t_transport).toLocaleString('id-ID', {minimumFractionDigits: 2})}` : '-' },
  ...
  return (
    <div className='bg-(--background-tar-color)'> 
        ...
        <main className="flex-1">
          ...
          <StatBoxUmranComponents onFilterChange={handleFilterChange} activeFilter={activeFilter} />
          </div>
        ...
        <div className="bg-white dshadow rounded-lg p-6">
          ...
          {loading ? (
                ...
                <div className="p-6 text-center bg-gray-50 rounded">
                  ...
                  <ButtonComponents 
                    variant='primary'
                    onClick={() => navigate('/tambah-user-umran')}
                  >
                    Add New User
                  </ButtonComponents>
                </div>
                ...
                <div className="p-6 text-center bg-gray-50 rounded">
                  <p className="text-gray-500">No data matches search term "{searchTerm}".</p>
                </div>
              ) : (
                <DataTable 
                  columns={columns}
      ...
      {isDeleteModalOpen && (
        <DeleteReasonComponents
          ...
          userName={userToDelete?.nama_lengkap_karyawan || ''}
        />
      )}
...
export default UmranEditProjectPage