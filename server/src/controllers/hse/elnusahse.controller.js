// Import the HSE history model
import { ElnusaHSEHistory } from "../../models/hse/elnusa_hse_history.model.js";
...
const updateHSEDataWithHistory = async (req, res) => {
    try {
        ...
        if (oldUserData.hse !== 'RIG EMR') {
            console.log(`User dengan ID ${id} bukan RIG EMR`);
            return res.status(403).json({
                success: false,
                message: 'Hanya user dengan status HSE RIG EMR yang dapat diupdate'
            });
        }
        ...
        const hseUpdateData = {
            ...
            // Ensure HSE field remains "RIG EMR"
            hse: 'RIG EMR'
        };

        // Check if HSE data changed significantly
        const isHSEChanged = ElnusaHSEHistory.isHSEDataChanged(oldUserData, hseUpdateData);
        ...
        try {
            ...
            const updatedUser = await Elnusa.updateUser(id, hseUpdateData);
            
            if (!updatedUser) {
                throw new Error('Gagal memperbarui data HSE');
            }
            
            // If HSE data was significantly changed, record in history
            if (isHSEChanged) {
                try {
                    // Prepare history data
                    const historyData = ElnusaHSEHistory.prepareHistoryData(oldUserData, hseUpdateData, modifiedBy);
                    
                    // Add record to history
                    await ElnusaHSEHistory.addHistory(historyData);
...
const getHSEHistory = async (req, res) => {
    try {
        ...
        // Ensure user exists and is a RIG EMR user
        const userExists = await Elnusa.getUserById(userId);
        if (!userExists) {
            return res.status(404).json({
                success: false,
                message: 'Karyawan tidak ditemukan'
        ...
        // Verify this is a RIG EMR user
        if (userExists.hse !== 'RIG EMR') {
            return res.status(403).json({
                success: false,
                message: 'Hanya user dengan status HSE RIG EMR yang dapat dilihat riwayatnya'
            });
        }

        // Get history records
        const history = await ElnusaHSEHistory.getAllHistoryByUserId(userId);
...
const getHSEHistoryStats = async (req, res) => {
    try {
        ...
        // Get statistics
        const stats = await ElnusaHSEHistory.getHSEChangeStats();
...
export const ElnusaHSEController = {
