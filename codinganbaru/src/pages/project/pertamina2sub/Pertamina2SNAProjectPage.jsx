...
const Pertamina2SNAProjectPage = () => {
  // State untuk data
  ...
  const fetchNonActiveData = async () => {
    ...
    try {
      ...
      try {
        const checkResult = await axios.get(`${API_BASE_URL}/regional2s/check-expired`, {
          headers: {
      ...
      const response = await axios.get(`${API_BASE_URL}/regional2s/na`, {
        headers: {
          'Accept': 'application/json',
      ...
      if (expiredWithoutSebabNA.length > 0) {
        ...
        for (const employee of expiredWithoutSebabNA) {
          try {
            await axios.put(`${API_BASE_URL}/regional2s/users/${employee.id}/na`, 
              { sebab_na: 'EOC' },
              {
                headers: {
                  'Content-Type': 'application/json',
        ...
        if (expiredWithoutSebabNA.length > 0) {
          const refreshResponse = await axios.get(`${API_BASE_URL}/regional2s/na`, {
            headers: {
              'Accept': 'application/json'
            },
            params: {
      ...
      try {
        const backupResponse = await axios.get(`${API_BASE_URL}/regional2s/users`, {
          headers: {
            'Accept': 'application/json'
          }
        });
  ...
  const manualCheckExpired = async () => {
    try {
      ...
      const updatePromises = expiredNoSebabNA.map(user => 
        axios.put(`${API_BASE_URL}/regional2s/users/${user.id}/na`, 
          { sebab_na: 'EOC' },
          {
            headers: {
              'Content-Type': 'application/json',
  ...
  const restoreUserToActive = async (id) => {
    try {
      ...
      const response = await axios.put(
        `${API_BASE_URL}/regional2s/users/${id}/restore`, 
        {},
        {
          headers: {
            'Content-Type': 'application/json',
...
- The contract end date must be in the future`;
    ...
    if (window.confirm(confirmMessage)) {
      ...
      try {
        ...
        axios.put(
          `${API_BASE_URL}/regional2s/users/${user.id}/restore`,
          {},
          {
            headers: {
              'Content-Type': 'application/json',
...
const formatWaktuSA = (dateTimeString) => {
  ...
  const handleExportAllData = () => {
    ...
    exportToExcel(filteredRecords, 'all_inactive_employee_data_regional2s');
  };
  ...
  const handleExportSelectedData = () => {
    ...
    exportToExcel(selectedRows, 'selected_inactive_employee_data_regional2s');
  };
...
export default Pertamina2SNAProjectPage