...
import { faHouse, faFileContract, faHelmetSafety, faArrowRightFromBracket, faBell, faSun, 
  faMagnifyingGlass, faFileAlt, faDownload, faUpload, faImage, faUserGroup } from '@fortawesome/free-solid-svg-icons';
import { faFile, faFileImage, faFileWord, faFilePdf, faFileExcel } from '@fortawesome/free-regular-svg-icons';
import View2SBudget from '../../../components/budget/View2SBudget';
...
const CReg2s = () => {
  ...
  const handleDataChange = (data) => {
    ...
    // Misalnya menyimpannya ke dalam state jika diperlukan
  };
  ...
  const fetchDataFromBackend = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`${API_BASE_URL}/regional2s/view`, {
        headers: {
      ...
      // For debugging file-related columns
      if (response.data && response.data.length > 0) {
      const fileFields = ['cv_filename', 'ijazah_filename', 'sertifikat_filename'];
      const fileData = response.data
        .filter(item => fileFields.some(field => item[field])) // Filter only items that have files
        .map(item => ({
          id: item.id,
          name: item.nama_lengkap,
          cv: item.cv_filename,
          diploma: item.ijazah_filename,
          certificate: item.sertifikat_filename
        }));
      }
      ...
      // Handle if the response is under a "data" property
      ...
      // Try fallback if the view endpoint fails
      try {
      const fallbackResponse = await axios.get(`${API_BASE_URL}/regional2s/users`, {
        headers: {
          'Accept': 'application/json'
        },
        timeout: 10000
      ...
      if (Array.isArray(fallbackResponse.data)) {
        // Filter only active employees (contracts not ended, no resignation reason)
        const activeUsers = fallbackResponse.data.filter(user => {
          if (!user) return false;

          // Active employee: no resignation reason and contract not yet expired
          const noResignationReason = !user.sebab_na;
          const contractNotExpired = user.kontrak_akhir ? 
            getDaysDifference(user.kontrak_akhir) >= 0 : true;

          return noResignationReason && contractNotExpired;
        });
  ...
  // StatBox component (integrated directly)
  const StatBox = () => {
  ...
  const columns = [
    ...
    { name: 'Employee Name', selector: row => row.nama_lengkap || '', sortable: true, width: "450px" },
      ...
      <div className='bg-(--background-tar-color)'> 
        <div className="flex">
            ...
            <div className='flex sticky top-0'>
              <aside className="w-70 bg-white text-black-70 p-4 flex flex-col h-screen overflow-y-auto">
                ...
                <nav className="mt-8 space-y-4">
                  {/* Link Dashboard */}
                  <Link to="/user-reg2s" className={getLinkClassName('/user-reg2s')}>
                    <FontAwesomeIcon icon={faHouse} className='text-lg'/>
                    <span className='text-sm'>Dashboard</span>
                  </Link>
                  
                  {/* Link Contract */}
                  <Link to="/contract-reg2s" className={getLinkClassName('/contract-reg2s')}>
                    <FontAwesomeIcon icon={faFileContract} className='text-lg'/>
                    <span className='text-sm'>Contract</span>
                  </Link>
                  
                  {/* Link HSE */}
                  <Link to="/data-hse-reg2s" className={getLinkClassName('/data-hse-reg2s')}>
          ...
          <div className="flex-1 flex flex-col h-screen ml-68 bg-(--background-tar-color) overflow-y-auto px-6 py-3">
            ...
            {/* Back button */}
            <div className="mb-4">
              <Link to='/user-reg2s' className="inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition">
                &lt; Back
              </Link>
            </div>
