...
export const Regional2Z7WContractHistory = {
    /**
    ...
    addHistory: async (historyData) => {
        try {
            ...
            const query = `
            INSERT INTO regional2z7w_contract_history 
            (${keys.join(', ')})
            VALUES (${placeholders})
            RETURNING *
            `;
    ...
    getHistoryByUserId: async (userId) => {
        try {
            ...
            SELECT EXISTS (
                ...
                AND table_name = 'regional2z7w_contract_history'
            );
            `;
            ...
            console.error("Table regional2z7w_contract_history does not exist");
            return [];
            }
            
            // Use parameterized query with better formatting
            const query = `
            SELECT * FROM regional2z7w_contract_history
            WHERE user_id = $1
            ...
            const diagnosticQuery = `SELECT COUNT(*) FROM regional2z7w_contract_history`;
            ...
            if (parseInt(diagnosticResult.rows[0].count) > 0) {
                ...
                const sampleQuery = `SELECT * FROM regional2z7w_contract_history LIMIT 1`;
    ...
    getAllHistoryByUserId: async (userId) => {
        try {
            ...
            const newHistoryQuery = `
                ...
                FROM regional2z7w_contract_history
                WHERE user_id = $1
            `;
