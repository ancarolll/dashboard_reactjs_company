...
export class Reg2z7dBudgetModel {
    ...
    try {
      ...
      const masterTableCheck = await pool.query(`
        SELECT EXISTS (
          ...
          WHERE table_name = 'reg2z7d_budget_master'
        );
      `);
      ...
      if (!masterTableCheck.rows[0].exists) {
        await pool.query(`
          CREATE TABLE reg2z7d_budget_master (
          ...
          CREATE TABLE IF NOT EXISTS reg2z7d_budget_project (
            id SERIAL PRIMARY KEY,
          ...
          CREATE TABLE IF NOT EXISTS reg2z7d_budget_history (
            id SERIAL PRIMARY KEY,
          ...
          CREATE INDEX IF NOT EXISTS idx_reg2z7d_budget_project_name ON reg2z7d_budget_project(project_name);
        `);
        console.log('Reg2Z7D Budget tables created successfully');
      }
    } catch (error) {
      console.error('Error initializing reg2z7d_budget tables:', error);
      throw error;
    }
  }
  ...
  static async createOrUpdateMaster(budgetData, username = 'system') {
    try {
      ...
      try {
        ...
        const checkQuery = `SELECT * FROM reg2z7d_budget_master`;
        ...
        if (checkResult.rows.length === 0) {
          ...
          const insertQuery = `
            INSERT INTO reg2z7d_budget_master (
              total_budget, current_remaining, total_termin1, total_termin2, total_termin3, total_termin4, status, remarks
          ...
          const updateQuery = `
            UPDATE reg2z7d_budget_master SET
              total_budget = $1,
              current_remaining = $2,
              total_termin1 = $3,
              total_termin2 = $4,
          ...
          if (oldTotalBudget !== newTotalBudget) {
            await client.query(`
              INSERT INTO reg2z7d_budget_history (
    ...
    try {
      ...
      try {
        ...
        const projectCheckQuery = `
          SELECT * FROM reg2z7d_budget_project WHERE ${projectData.id ? 'id = $1' : 'project_name = $1'}
        `;
        ...
        if (projectCheckResult.rows.length === 0) {
          // Insert new project
          const insertQuery = `
            INSERT INTO reg2z7d_budget_project (
              project_name, total_budget, termin1, termin2, termin3, termin4, 
          ...
          const updateQuery = `
            UPDATE reg2z7d_budget_project SET
              project_name = $1,
              total_budget = $2,
              termin1 = $3,
              termin2 = $4,
  ...
  static async updateMasterBudgetTotals(client) {
    ...
    // Get master budget
    const masterQuery = `SELECT * FROM reg2z7d_budget_master`;
    ...
    if (masterResult.rows.length === 0) {
      ...
      await client.query(`
        INSERT INTO reg2z7d_budget_master (
          ...
          total_termin3, total_termin4, status, remarks
        ) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
      `, [
  ...
  static async getMasterBudget() {
    try {
      const query = `SELECT * FROM reg2z7d_budget_master`;
  ...
  static async getAllProjects(filters = {}, page = 1, limit = 50) {
    try {
      let query = `SELECT * FROM reg2z7d_budget_project WHERE 1=1`;
      const values = [];
      let paramCount = 1;
      ...
      let countQuery = `SELECT COUNT(*) FROM reg2z7d_budget_project WHERE 1=1`;
      let countValues = [];
  ...
  static async deleteProject(id, username = 'system') {
    try {
      ...
      try {
        ...
        const projectQuery = `SELECT * FROM reg2z7d_budget_project WHERE id = $1`;
        ...
        // Delete the project
        const deleteQuery = `DELETE FROM reg2z7d_budget_project WHERE id = $1 RETURNING *`;
  ...
  static async getBudgetHistory() {
    try {
      const query = `
        SELECT * FROM reg2z7d_budget_history 
  ...
  static async getFilterOptions() {
    try {
      const projectNameQuery = `SELECT DISTINCT project_name FROM reg2z7d_budget_project ORDER BY project_name`;
  ...
  static async getBudgetSummary() {
    try {
      ...
      // Get all projects
      const projectsQuery = `SELECT * FROM reg2z7d_budget_project ORDER BY created_at`;
    ...
    try {
      ...
      try {
        ...
        for (const row of data) {
          ...
          // Check if project already exists
          const projectCheckQuery = `
            SELECT * FROM reg2z7d_budget_project WHERE project_name = $1
          `;
...
export default Reg2z7dBudgetModel;