...
export const Regional2xContractHistory = {
   ...
   */
    addHistory: async (historyData) => {
        ...
        const query = `
            INSERT INTO regional2x_contract_history 
            (${keys.join(', ')})
            VALUES (${placeholders})
            RETURNING *
        `;
    ...
    getHistoryByUserId: async (userId) => {
        ...
        const checkTableQuery = `
            ...
            AND table_name = 'regional2x_contract_history'
            );
        `;
        ...
        if (!tableExists.rows[0].exists) {
            console.error("Table regional2x_contract_history does not exist");
            return [];
        }
        
        // Use parameterized query with better formatting
        const query = `
            SELECT * FROM regional2x_contract_history
            WHERE user_id = $1
        ...
        if (result.rowCount === 0) {
            const diagnosticQuery = `SELECT COUNT(*) FROM regional2x_contract_history`;
            ...
            const sampleQuery = `SELECT * FROM regional2x_contract_history LIMIT 1`;
    ...
    getAllHistoryByUserId: async (userId) => {
        ...
        if (newTableExists.rows[0].exists) {
            ...
            FROM regional2x_contract_history
            WHERE user_id = $1
            `;
