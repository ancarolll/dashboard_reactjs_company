...
import fs from 'fs';
import { Regional2sContractHistory } from "./regional2s_contract_history.model.js";
...
export const Regional2S = {
    ...
    _cleanNumericData: (data) => {
        ...
        // Definisi kolom numeric (sesuaikan dengan struktur tabel Regional2s)
    ...
    _createInsertQuery: (data) => {
        ...
        // Bersihkan data
        const cleanData = Regional2S._cleanData(dataWithoutId);
        ...
        const sql = `
            INSERT INTO project_pertamina_ep_regional2_subsurface (${filteredKeys.join(', ')})
            VALUES (${placeholders})
            RETURNING *
        `;
    ...
    _createUpdateQuery: (id, data) => {
        const cleanData = Regional2S._cleanData(data);
        ...
        const fileFields = [
            ...
            // Tambahan kolom file baru untuk regional2s
        ...
        const sql = `
            UPDATE project_pertamina_ep_regional2_subsurface
            SET ${setClause}
    ...
    _createSelectQuery: (conditions = {}, orderBy = 'id ASC') => {
        ...
        // Buat query SQL dinamis
        const sql = `
            SELECT * FROM project_pertamina_ep_regional2_subsurface
            ${whereClause}
            ORDER BY ${orderBy}
        `;
    ...
    _createSelectColumnsQuery: (columns = [], conditions = {}, orderBy = 'id ASC') => {
        ...
        // Buat query SQL dinamis
        const sql = `
            SELECT ${columnsClause} FROM project_pertamina_ep_regional2_subsurface
            ${whereClause}
            ORDER BY ${orderBy}
        `;
    ...
    _createDeleteQuery: (conditions = {}) => {
        ...
        // Buat query SQL dinamis
        const sql = `
            DELETE FROM project_pertamina_ep_regional2_subsurface
            ${whereClause}
            RETURNING *
        `;
    ...
    // Fungsi untuk cek apakah kolom dokumen ada di database (UPDATED)
    checkDocumentColumns: async () => {
        try {
            const sql = `
                SELECT column_name FROM information_schema.columns 
                WHERE table_name = 'project_pertamina_ep_regional2_subsurface' 
                AND column_name IN (
                    'cv_filename', 'cv_filepath', 'cv_mimetype', 'cv_filesize',
                    'ijazah_filename', 'ijazah_filepath', 'ijazah_mimetype', 'ijazah_filesize',
                    'sertifikat_filename', 'sertifikat_filepath', 'sertifikat_mimetype', 'sertifikat_filesize',
            ...
            // Regional2S memiliki 40 kolom (10 tipe dokumen × 4 properti per dokumen)
    ...
    // Fungsi untuk menambahkan kolom dokumen jika belum ada (UPDATED)
    addDocumentColumns: async () => {
        try {
            const hasColumns = await Regional2S.checkDocumentColumns();
            ...
            const sql = `
                ALTER TABLE project_pertamina_ep_regional2_subsurface
    ...
    // Fungsi untuk mengupload dokumen (UPDATED)
    uploadDocument: async (userId, docType, fileInfo) => {
        try {
            ...
            console.log(`Mengunggah dokumen ${docType} untuk user ID: ${userId}`);
            console.log('Informasi file:', {
                nama: fileInfo.originalname,
            ...
            await Regional2S.addDocumentColumns();
            
            // Validasi tipe dokumen yang sesuai untuk Regional2S
            const validDocTypes = [
            ...
            if (!validDocTypes.includes(docType)) {
                throw new Error(`Tipe dokumen tidak valid untuk Regional2S: ${docType}`);
            }
            
            // Cek user
            const checkUserResult = await pool.query(
                `SELECT id FROM project_pertamina_ep_regional2_subsurface WHERE id = $1`,
                [userId]
            );
            ...
            const oldFileResult = await pool.query(
                `SELECT ${docType}_filepath FROM project_pertamina_ep_regional2_subsurface WHERE id = $1`,
                [userId]
            );
            
            const oldFilePath = oldFileResult.rows[0]?.[`${docType}_filepath`];
            ...
            console.log(`Berhasil menyimpan info file ${docType} di database`);
            return updateResult.rows[0];
        } catch (error) {
            console.error(`Error pada uploadDocument:`, error);
            throw error;
        }
    },

    // Fungsi untuk mendapatkan info dokumen (UPDATED)
    getDocumentInfo: async (userId, docType) => {
        try {
            ...
            // Validate document type untuk Regional2S
            const validDocTypes = [
                'cv', 'ijazah', 'sertifikat', 'pkwt',
                'no_ktp', 'no_kk', 'no_npwp', 'bpjs_kesehatan', 'bpjstk', 'no_rekening'
            ];
            ...
            if (!validDocTypes.includes(docType)) {
                throw new Error('Invalid document type for Regional2S');
            }
            ...
            const sql = `
                ...
                FROM project_pertamina_ep_regional2_subsurface 
                WHERE id = $1
            `;
    ...
    // Fungsi untuk menghapus dokumen dari database dan filesystem (UPDATED)
    deleteDocument: async (userId, docType) => {
        try {
            // Validasi tipe dokumen untuk Regional2S
            const validDocTypes = [
                'cv', 'ijazah', 'sertifikat', 'pkwt',
                'no_ktp', 'no_kk', 'no_npwp', 'bpjs_kesehatan', 'bpjstk', 'no_rekening'
            ];
    ...
    // Fungsi untuk memeriksa apakah tabel sertifikat ada
        ...
        try {
            const sql = `
                SELECT EXISTS (
                    ...
                    AND table_name = 'regional2s_sertifikat'
                );
            `;
            const result = await pool.query(sql);
    ...
    createSertifikatTable: async () => {
        try {
            ...
            const tableExists = await Regional2S.checkSertifikatTableExists();
            
            if (tableExists) {
                console.log("Table regional2s_sertifikat already exists");
                return true;
            }
            
            // Buat tabel jika belum ada
            const sql = `
                CREATE TABLE regional2s_sertifikat (
                    id SERIAL PRIMARY KEY,
                    user_id INTEGER NOT NULL REFERENCES project_pertamina_ep_regional2_subsurface(id) ON DELETE CASCADE,
                ...
                CREATE INDEX IF NOT EXISTS idx_regional2s_sertifikat_user_id ON regional2s_sertifikat(user_id);
            `;
            
            await pool.query(sql);
            console.log("Table regional2s_sertifikat created successfully");
            return true;
        } catch (error) {
    ...
    addSertifikat: async (sertifikatData) => {
        try {
            ...
            const sql = `
                INSERT INTO regional2s_sertifikat (
    ...
    getSertifikatByUserId: async (userId) => {
        try {
            ...
            const sql = `
                SELECT * FROM regional2s_sertifikat
                WHERE user_id = $1
    ...
    deleteSertifikat: async (id) => {
        try {
            ...
            const sql = `
                DELETE FROM regional2s_sertifikat
                WHERE id = $1
                RETURNING *
            `;
        ...
        try {
            ...
            for (let i = 0; i < usersData.length; i++) {
                ...
                try {
                    ...
                    const normalizedData = Regional2S.validateAndNormalizeUserData(userData);
                    
                    // Siapkan query untuk insert
                    const { sql, values } = Regional2S._createInsertQuery(normalizedData);
        ...
        try {
            const sql = `
                SELECT EXISTS (
                    SELECT FROM information_schema.tables 
                    WHERE table_schema = 'public' 
                    AND table_name = 'project_pertamina_ep_regional2_subsurface'
                );
            `;
            const result = await pool.query(sql);
            return result.rows[0].exists;
    ...
    resetSequence: async () => {
        try {
            ...
            const sequenceQuery = `
                SELECT pg_get_serial_sequence('project_pertamina_ep_regional2_subsurface', 'id') as seq_name;
            `;
            const sequenceResult = await pool.query(sequenceQuery);
            const sequenceName = sequenceResult.rows[0]?.seq_name;
            
            if (!sequenceName) {
                console.error('Cannot find sequence for project_pertamina_ep_regional2_subsurface.id');
                return false;
            }
            
            // Dapatkan nilai maksimal ID yang ada
            const maxIdQuery = `SELECT COALESCE(MAX(id), 0) + 1 as next_id FROM project_pertamina_ep_regional2_subsurface`;
    ...
    checkTableStructure: async () => {
        try {
            ...
            const tableExists = await Regional2S.checkTableExists();
            
            if (!tableExists) {
                console.error("Table project_pertamina_ep_regional2_subsurface does not exist!");
    ...
    getAll: async () => {
        try {
            ...
            if (result.rowCount === 0) {
                ...
                const allRows = await pool.query('SELECT id, nama_lengkap, kontrak_akhir, sebab_na FROM project_pertamina_ep_regional2_subsurface LIMIT 10');
            ...
            if (error.code === '42P01') {
                console.error("Table project_pertamina_ep_regional2_subsurface doesn't exist");
                return [];
            }
            throw error;
        }
    ...
    restoreUser: async (id) => {
        try {
            ...
            const userSql = `SELECT * FROM project_pertamina_ep_regional2_subsurface WHERE id = $1`;
            ...
            const preservedData = {
                tanggal_lahir: user.tanggal_lahir // Simpan tanggal lahir
                // Tambahkan field lain yang perlu dipertahankan
            };
            ...
            const updateData = { 
                sebab_na: null,
                tanggal_lahir: preservedData.tanggal_lahir // Eksplisit pertahankan tanggal lahir
            };
    
            // Update data
            const { sql, values } = Regional2S._createUpdateQuery(id, updateData);
            const result = await pool.query(sql, values);
    ...
    addUser: async (userData) => {
        try {
            await Regional2S.resetSequence();
            ...
            const { sql, values } = Regional2S._createInsertQuery(userData);
    ...
    getUserById: async (id) => {
        try {
            ...
            const { sql, values } = Regional2S._createSelectQuery({ id });
            const result = await pool.query(sql, values);
            return result.rows[0];
        } catch (error) {
    ...
    updateUser: async (id, userData) => {
        try {
            ...
            const oldData = await Regional2S.getUserById(id);
            if (!oldData) {
            ...
            const cleanedUserData = Regional2S._cleanData(userData);
            ...
            // Update data user
            const { sql, values } = Regional2S._createUpdateQuery(id, cleanedUserData);
            const result = await pool.query(sql, values);
        
            // Jika kontrak berubah, simpan history
            if (isKontrakChanged) {
                await Regional2S.saveKontrakHistory(id, oldData, cleanedUserData);
            }
    ...
    saveKontrakHistory: async (userId, oldData, newData) => {
        try {
            ...
            // Periksa apakah tabel regional2s_contract_history ada (bukan hse_regional2s_history)
            const sql = `
                SELECT EXISTS (
                    SELECT FROM information_schema.tables 
                    WHERE table_schema = 'public' 
            ...
            if (!tableExists.rows[0].exists) {
                ...
                console.log("Cannot save contract history as regional2s_contract_history table doesn't exist");
                return null;
            }
            
            // Siapkan data dengan struktur kolom yang benar
            const historyData = {
                user_id: userId,
                nama_karyawan: oldData.nama_lengkap || 'Unknown',
                ...
                modified_by: 'system',
                waktu_perubahan: new Date() // Gunakan waktu_perubahan, bukan tanggal_perubahan
            };
            ...
            const historySql = `
                INSERT INTO regional2s_contract_history (${keys.join(', ')})
                VALUES (${placeholders})
                RETURNING *
            `;
    ...
    getKontrakHistory: async (userId) => {
        try {
            ...
            // Periksa apakah tabel hse_regional2s_history ada
            const sql = `
                SELECT EXISTS (
                    SELECT FROM information_schema.tables 
                    WHERE table_schema = 'public' 
            ...
            if (!tableExists.rows[0].exists) {
                console.error("Table hse_regional2s_history does not exist!");
                return [];
            }
    ...
    deleteUser: async (id) => {
        try {
            ...
            // Check if hse_regional2s_history table exists before attempting to delete history records
            ...
            if (historyTableExists.rows[0].exists) {
                const historySql = `DELETE FROM hse_regional2s_history WHERE user_id = $1`;
                await pool.query(historySql, [id]);
            }
        
            // Then delete the user
            const { sql, values } = Regional2S._createDeleteQuery({ id });
            const result = await pool.query(sql, values);
            return result.rows[0];
        } catch (error) {
    ...
    getViewData: async () => {
        try {
            ...
            const columns = [
                ...
                'pkwt_filename', 'pkwt_filepath', 'pkwt_mimetype', 'pkwt_filesize',
                // Tambahkan kolom dokumen tambahan
                'no_ktp_filename', 'no_ktp_filepath', 'no_ktp_mimetype', 'no_ktp_filesize',
                'no_kk_filename', 'no_kk_filepath', 'no_kk_mimetype', 'no_kk_filesize',
                'no_npwp_filename', 'no_npwp_filepath', 'no_npwp_mimetype', 'no_npwp_filesize',
                'bpjs_kesehatan_filename', 'bpjs_kesehatan_filepath', 'bpjs_kesehatan_mimetype', 'bpjs_kesehatan_filesize',
            ...
            const sql = `
                SELECT ${columnsStr} FROM project_pertamina_ep_regional2_subsurface
    ...
    getPersonalData: async (id) => {
        try {
            ...
            const columns = [
                'id', 'nama_lengkap', 'alamat_domisili', 'alamat_ktp', 'asuransi_lainnya', 'cv',
                'tanggal_lahir', 'tempat_lahir', 'no_telpon', 'kontak_darurat',
                'nama_ibu_kandung', 'jenis_kelamin', 'status_pernikahan',
                'email', 'nama_institute_pendidikan', 'jurusan'
            ];
            
            const { sql, values } = Regional2S._createSelectColumnsQuery(columns, { id });
            const result = await pool.query(sql, values);
            return result.rows[0];
        } catch (error) {
    ...
    updateUserWithHistory: async (id, userData, modifiedBy = 'system') => {
        try {
            ...
            const isContractChanged = Regional2S._isContractChanged(oldUserData, userData);
            
            // Begin transaction
            await pool.query('BEGIN');
            
            // Update user data
            const updateResult = await Regional2S.updateUser(id, userData);
            
            // If contract changed, record the change history
            if (isContractChanged) {
                try {
                    await Regional2sContractHistory.addHistory({
                        user_id: id,
    ...
    getAdminData: async (id) => {
        try {
            ...
            const columns = [
                ...
                'bpjs_kesehatan_suami_istri', 'bpjs_anak1', 'bpjs_anak2', 'bpjs_anak3',
                'bpjstk', 'no_npwp', 'no_rekening', 'nama_rekening',
                'nama_bank', 'gaji_net'
            ];
    ...
    getHSEData: async () => {
        try {
            ...
            if (!tableExists) {
                console.error("Tabel project_pertamina_ep_regional2_subsurface tidak ditemukan!");
                return [];
            }
            
            // Ambil semua data karyawan dengan kolom-kolom yang relevan untuk HSE
            const sql = `
                SELECT id, nama_lengkap, jabatan, sebab_na, nik_vendor, no_kontrak,
            ...
            if (error.code === '42P01') {
                console.error("Tabel project_pertamina_ep_regional2_subsurface tidak ada");
                return [];
            }
            throw error;
        }
