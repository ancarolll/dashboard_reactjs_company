import { Regional2Z7W } from "../../models/perusahaan/regional2Z7W.model.js"
import fs from 'fs';
import path from 'path';
import { Regional2Z7WContractHistory } from "../../models/perusahaan/regional2Z7W_contract_history.model.js";
import Papa from 'papaparse';
import * as XLSX from 'xlsx';
import pool from "../../config/db.js";

/**
 * Update the validateDocType function to include the new document types for regional2Z7W
 */
...
export const Regional2Z7WController = {
    ...
    try {
        ...
        if (req.query.format === 'json') {
            const dataKaryawan = await Regional2Z7W.getAll();
            return res.json(dataKaryawan);
        }
        
        res.redirect('/api/regional2Z7W/edit');
    } catch (error) {
    ...
    checkDbStructure: async (req, res) => {
        try {
            const tableStructure = await Regional2Z7W.checkTableStructure();
            res.json({
    ...
    getEditPage: async (req, res) => {
        try {
            ...
            res.render('Regional2Z7WEditProjectPage', {
                title: 'Edit Project Regional 2 Zona 7 WOPDM',
    ...
    getViewPage: async (req, res) => {
        try {
            ...
            const dataKaryawan = await Regional2Z7W.getViewData();
            ...
            res.render('Regional2Z7WViewProjectPage', {
                title: 'View Project Regional 2 Zona 7 WOPDM',
                karyawan: dataKaryawan,
                debugMode: process.env.NODE_ENV !== 'production'
            });
            
            console.log('Regional2Z7WViewProjectPage rendered successfully');
        } catch (error) {
    ...
    getNaPage: async (req, res) => {
        try {
            ...
            const dataKaryawan = await Regional2Z7W.getNAUsers();
            ...
            res.render('Regional2Z7WNAProjectPage', {
                title: 'NA Project Regional 2 Zona 7 WOPDM',
                karyawan: dataKaryawan,
                debugMode: process.env.NODE_ENV !== 'production'
            });
        } catch (error) {
    ...
    getTambahUserForm: async (req, res) => {
        try {
            ...
            res.render('TambahUserRegional2Z7W', {
                title: 'Tambah User Regional 2 Zona 7 WOPDM',
                karyawan: {},
                debugMode: process.env.NODE_ENV !== 'production'
            });
            
            console.log('TambahUserRegional2Z7W form rendered successfully');
        } catch (error) {
    ...
    */
    getEditUserForm: async (req, res) => {
        try {
            ...
            const dataKaryawan = await Regional2Z7W.getUserById(id);
            ...
            res.render('TambahUserRegional2Z7W', {
                title: 'Edit User Regional 2 Zona 7 WOPDM',
                karyawan: dataKaryawan,
                debugMode: process.env.NODE_ENV !== 'production'
            });
    ...
    addUser: async (req, res) => {
        ...
        await Regional2Z7W.resetSequence();
        ...
        const result = await Regional2Z7W.addUser(userData);
        ...
        if (result.sebab_na) {
            res.redirect('/api/regional2Z7W/na');
        } else {
            res.redirect('/api/regional2Z7W/edit');
        }
        } catch (error) {
    ...
    updateUser: async (req, res) => {
        ...
        const fileFields = [
          ...
          fileFields.forEach(field => {
            ...
            const oldUserData = await Regional2Z7W.getUserById(userId);
            if (!oldUserData) {
            ...
            const result = await Regional2Z7W.updateUserWithHistory(userId, userData, modifiedBy);
    ...
    */
    setUserInactive: async (req, res) => {
        try {
            ...
            const userExists = await Regional2Z7W.getUserById(id);
            if (!userExists) {
            return res.status(404).json({ 
                success: false, 
                message: 'Karyawan tidak ditemukan' 
            ...
            try {
            const result = await Regional2Z7W.updateUserStatus(id, sebab_na);
    ...
    */
    ...
    try {
        ...
        const userData = await Regional2Z7W.getUserById(id);
        if (!userData) {
        ...
        const restoredUser = await Regional2Z7W.restoreUser(id);
        ...
        previous_status: {
         ...
         // UBAH INSTRUKSI - hilangkan referensi masa depan
        instructions: 'To reactivate user, please update contract dates and contract number'
    };
        ...
        res.redirect(`/api/regional2Z7W/edit-user/${id}?restored=true&message=Update contract dates and contract number`);
    } catch (error) {
    ...
    deleteUser: async (req, res) => {
        try {
            ...
            const result = await Regional2Z7W.deleteUser(id);
    ...
    uploadDocument: async (req, res) => {
      try {
        ...
        const userExists = await Regional2Z7W.getUserById(userIdInt);
        if (!userExists) {
          return res.status(404).json({ 
        ...
        const result = await Regional2Z7W.uploadDocument(userId, docType, req.file);
    ...
    downloadDocument: async (req, res) => {
        try {
            ...
            const fileInfo = await Regional2Z7W.getDocumentInfo(userId, docType);
    ...
    getDocumentFile: async (req, res) => {
      try {
        ...
        // Get employee data
        const employee = await Regional2Z7W.getUserById(userId);
        if (!employee) {
        ...
        const possiblePaths = [
          ...
          path.join('D:/dashboardadmin/server/src/uploads/regional2z7w-files', path.basename(normalizedPath))
        ];
    ...
    */
    checkExpiredContracts: async (req, res) => {
        try {
            ...
            let expiredUsers = [];
            try {
            expiredUsers = await Regional2Z7W.checkExpiredKontrak();
            } catch (checkError) {
        ...
        for (const user of expiredUsers) {
            try {
            const updatedUser = await Regional2Z7W.updateUserStatus(user.id, 'EOC');
    ...
    */
    updateUserWithHistory: async (req, res) => {
        ...
        const updatedUser = await Regional2Z7W.updateUser(id, userData);
        
        // Cek apakah contract history perlu diperbarui
        const oldUser = await Regional2Z7W.getUserById(id);
        const contractChanged = 
        ...
        if (contractChanged) {
            try {
            // Periksa apakah model Regional2Z7WContractHistory ada
            if (typeof Regional2Z7WContractHistory !== 'undefined') {
                await Regional2Z7WContractHistory.addHistory({
                user_id: id,
                ...
                console.warn('Regional2Z7WContractHistory not defined, skipping history recording');
                // Gunakan alternatif Regional2Z7W.saveKontrakHistory jika ada
                if (typeof Regional2Z7W.saveKontrakHistory === 'function') {
                await Regional2Z7W.saveKontrakHistory(id, oldUser, userData);
                }
            }
            } catch (historyError) {
    ...
    */
    ...
    const history = await Regional2Z7WContractHistory.getAllHistoryByUserId(userId);
    ...
    */
    getContractChangeStats: async (req, res) => {
        try {
        const stats = await Regional2Z7WContractHistory.getContractChangeStats();
    ...
    getKontrakHistory: async (req, res) => {
        ...
        // Panggil method getKontrakHistory dari model Regional2Z7W
        const history = await Regional2Z7W.getKontrakHistory(userId);
   ...
   */
    getPersonalData: async (req, res) => {
        try {
            const id = req.params.id;
            const personalData = await Regional2Z7W.getPersonalData(id);
    ...
    */
    deleteDocument: async (req, res) => {
        ...
        await Regional2Z7W.deleteDocument(userId, docType);
    ...
    */
    getCSVTemplate: async (req, res) => {
        try {
        // Definisi header CSV (disesuaikan dengan skema regional2Z7W)
        const headers = [
            "nama_lengkap", "jabatan", "no_kontrak", 
            "kontrak_awal", "kontrak_akhir", "tempat_lahir", "tanggal_lahir", "status_pernikahan", "alamat_domisili", "no_telepon", 
            "kontak_darurat", "nama_kontak_darurat", "email", "paket_tender",
            "no_ktp", "no_kk", "jenis_kelamin", "alamat_ktp", "nik_vendor", "gaji_net",
        ...
        const exampleData = [
            ...
            "12/11/2023", "31/12/2025", "Makassar", "10/11/1990", "TK/0", "Jl. Contoh No. 123", "08123456789",
            "08123456788", "Jane Doe", "john.doe@example.com", "Paket ABC",
            "1234567890123456", "1234567890", "Laki-laki", "Jl. KTP No. 123", "V-123456", "5000000",
            "12345678", "87654321", "12345679", "12345680", "12345681", "12345682", "Keterangan BPJS", "12345682",
            "12.345.678.9-012.345", "1234567890", "Bank Mandiri",
        ...
        res.setHeader('Content-Disposition', 'attachment; filename=template_data_karyawan_regional2Z7W.csv');
    ...
    */
    uploadBulkData: async (req, res) => {
        ...
        const validation = Regional2Z7W.validateCSVData(parsedData);
        ...
        const processResult = await Regional2Z7W.processBulkInsert(parsedData);
    ...
    */
    getAdminData: async (req, res) => {
        try {
            const id = req.params.id;
            const adminData = await Regional2Z7W.getAdminData(id);
    ...
    getCertificationData: async (req, res) => {
        try {
            const id = req.params.id;
            const certificationData = await Regional2Z7W.getCertificationData(id);
    ...
    getHSEData: async (req, res) => {
        ...
        const dataHSE = await Regional2Z7W.getHSEData();
    ...
    updateHSEDataWithHistory: async (req, res) => {
        ...
        const oldUserData = await Regional2Z7W.getUserById(id);
        if (!oldUserData) {
            console.log(`User dengan ID ${id} tidak ditemukan`);
            return res.status(404).json({
            success: false,
        ...
        const isHSEChanged = Regional2Z7WHSEHistory.isHSEDataChanged(oldUserData, hseUpdateData);
        ...
        try {
            ...
            const updatedUser = await Regional2Z7W.updateUser(id, hseUpdateData);
            ...
            try {
                ...
                const historyData = Regional2Z7WHSEHistory.prepareHistoryData(oldUserData, hseUpdateData, modifiedBy);
                
                // Tambahkan catatan ke riwayat
                await Regional2Z7WHSEHistory.addHistory(historyData);
    ...
    getHSEHistory: async (req, res) => {
        ...
        // Pastikan user ada
        const userExists = await Regional2Z7W.getUserById(userId);
        if (!userExists) {
            return res.status(404).json({
            success: false,
            message: 'Karyawan tidak ditemukan'
        ...
        const history = await Regional2Z7WHSEHistory.getAllHistoryByUserId(userId);
    ...
    getHSEHistoryStats: async (req, res) => {
        ...
        // Dapatkan statistik
        const stats = await Regional2Z7WHSEHistory.getHSEChangeStats();
    ...
    // CERTIFICATE MANAGEMENT FUNCTIONS - Same as Regional4

    // Membuat tabel sertifikat jika belum ada
    createSertifikatTable: async (req, res) => {
    try {
        await Regional2Z7W.createSertifikatTable();
    ...
    try {
        ...
        // Add the certificate
        const result = await Regional2Z7W.addSertifikat(sertifikatData);
    ...
    try {
        ...
        // Get certificates
        const result = await Regional2Z7W.getSertifikatByUserId(userId);
    ...
    try {
        ...
        // Get the certificate
        const result = await Regional2Z7W.getSertifikatById(id);
    ...
    try {
        ...
        const result = await Regional2Z7W.updateSertifikat(id, sertifikatData);
    ...
    try {
        ...
        const result = await Regional2Z7W.deleteSertifikat(id);
