...
const ElnusaHSEPage = () => {
  ...
  const fetchDataFromBackend = async () => {
      ...
      try {
      const response = await axios.get(`${API_BASE_URL}/elnusa/hse-data`, {
          headers: {
  ...
  const handleEdit = (user) => {
      ...
      navigate(`/hse-elnusa-form/${user.id}`, { 
      state: { 
          userData: {...user}
      } 
      });
  ...
  useEffect(() => {
      ...
      if (location.state?.editedUser && location.state?.updatedUser) {
          setMessage({
        text: `HSE data for ${location.state.updatedUser.nama_karyawan} successfully updated`,
        type: 'success'
          });
      }
      
      // Clear state routing untuk mencegah refresh berulang
      navigate('/hse-elnusa', { replace: true, state: {} });
      }
  ...
  // Table column definitions with sortable priority
  const mcuColumns = [
      { 
          name: 'Action', 
          cell: row => (
      ...
      { 
          ...
          cell: row => {
              const daysRemaining = getDaysDifference(row.akhir_mcu);
              return (
      ...
      { name: 'Employee Name', selector: row => row.nama_karyawan || '', sortable: true, width: "400px" },
      { name: 'NIK', selector: row => row.no_ktp || '', sortable: true, width: "250px" },
      { name: 'NIK Elnusa', selector: row => row.nik_elnusa || '', sortable: true, width: "250px" },
      ...
      { name: 'Domicile address', selector: row => row.alamat_lengkap_domisili || '', sortable: true, width: "1000px" }
  ];
  ...
  const hseColumns = [
      ...
      { 
          ...
          cell: row => {
              const daysRemaining = getDaysDifference(row.akhir_hsepassport);
              return (
                  <div className={`px-3 py-1 rounded-full ${getStatusColor(daysRemaining)}`}>
                      {getStatusText(daysRemaining)}
                  </div>
  ...
  const simlColumns = [
      ...
      { 
          ...
          cell: row => {
              const daysRemaining = getDaysDifference(row.akhir_siml);
              return (
                  <div className={`px-3 py-1 rounded-full ${getStatusColor(daysRemaining)}`}>
                      {getStatusText(daysRemaining)}
                  </div>
  ...
  const prepareDataForExport = (data, tableType) => {
      ...
      if (tableType === 'mcu') {
          ...
          alamat_domisili: newItem.alamat_lengkap_domisili || ''
          };
      } 
...
const ColorLegend = () => {
  ...
  exportToExcel(mcuFilteredData, 'mcu_data_elnusa_rig_emr', 'mcu');
  };
  
  const handleExportHSE = () => {
  exportToExcel(hseFilteredData, 'hse_passport_data_elnusa_rig_emr', 'hse');
  };
  
  const handleExportSIM = () => {
  exportToExcel(simlFilteredData, 'sim_data_elnusa_rig_emr', 'siml');
  };
    ...
    <div className='bg-(--background-tar-color)'> 
        <div className="flex">
            ...
            <main className="flex-1">
                <h1 className="text-2xl font-bold mb-4 mt-2 text-center">Elnusa RIG EMR HSE</h1>
                ...
                <>
                    ...
                    <div className="bg-white shadow rounded-lg p-6 mb-6">
                        ...
                        striped
                        responsive
                        defaultSortFieldId={2} // Default sort by MCU Status (2nd column, index starts at 1)
                    ...
                    <div className="bg-white shadow rounded-lg p-6 mb-6">
                        ...
                        striped
                        responsive
                        defaultSortFieldId={2} // Default sort by HSE Status (2nd column)
                        defaultSortAsc={true}  // Ascending order (lower priority values first)
                        noDataComponent={
                            <div className="p-4 text-center text-gray-500">
                    ...
                    <div className="bg-white shadow rounded-lg p-6 mb-6">
                        ...
                        striped
                        responsive
                        defaultSortFieldId={2} // Default sort by SIML Status (2nd column)
                        defaultSortAsc={true}  // Ascending order (lower priority values first)
                        noDataComponent={
                            <div className="p-4 text-center text-gray-500">
