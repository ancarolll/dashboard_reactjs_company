...
const ElnusaNAProjectPage = () => {  
  ...
  const fetchNonActiveData = async () => {
    ...
    try {
      const checkResult = await axios.get(`${API_BASE_URL}/elnusa/check-expired`, {
        headers: {
      ...
      console.error('Failed to check for expired contracts:', checkError);
      ...
      const response = await axios.get(`${API_BASE_URL}/elnusa/na`, {
        headers: {
          'Accept': 'application/json',
    ...
    if (response.data.length > 0) {
      ...
      if (expiredWithoutSebabNA.length > 0) {
        ...
        for (const employee of expiredWithoutSebabNA) {
          try {
            await axios.put(`${API_BASE_URL}/elnusa/users/${employee.id}/na`, 
              { sebab_na: 'EOC' },
              {
                headers: {
                  'Content-Type': 'application/json',
        ...
        if (expiredWithoutSebabNA.length > 0) {
          const refreshResponse = await axios.get(`${API_BASE_URL}/elnusa/na`, {
            headers: {
              'Accept': 'application/json'
            },
            params: {
      ...
      console.error('Error fetching NonActive employee data:', err);
      setError(err.message || 'An error occurred while fetching NonActive employee data');
      
      // Jika gagal mengambil dari endpoint /na, coba dengan metode backup
      try {
        const backupResponse = await axios.get(`${API_BASE_URL}/elnusa/users`, {
          headers: {
            'Accept': 'application/json'
          }
        });
  ...
  const manualCheckExpired = async () => {
    try {
      ...
      const updatePromises = expiredNoSebabNA.map(user => 
        axios.put(`${API_BASE_URL}/elnusa/users/${user.id}/na`, 
          { sebab_na: 'EOC' },
          {
            headers: {
              'Content-Type': 'application/json',
      ...
      console.error('Error saat pengecekan manual:', err);
  ...
  const restoreUserToActive = async (id) => {
    try {
      ...
      const response = await axios.put(
        `${API_BASE_URL}/elnusa/users/${id}/restore`, 
        {},
        {
          headers: {
            'Content-Type': 'application/json',
...
const formatWaktuSA = (dateTimeString) => {
  ...
  const filteredRecords = React.useMemo(() => {
   ...
   // Fungsi handle restore - perbaikan untuk navigasi ke halaman edit
   const handleRestore = (user) => {
    ...
    const confirmMessage = `Are you sure you want to restore employee "${user.nama_karyawan}" ke status aktif?
  
  IMPORTANT: 
  - This action will delete the inactive reason (${user.sebab_na || '-'})
  - You MUST change the contract date and contract number to reactivate the employee
  - The contract end date must be in the future`;
    ...
    if (window.confirm(confirmMessage)) {
      ...
      try {
        ...
        axios.put(
          `${API_BASE_URL}/elnusa/users/${user.id}/restore`,
          {},
          {
            headers: {
              'Content-Type': 'application/json',
        ...
        .then(response => {
          ...
          // Buat salinan user data yang lengkap
          const userData = {...user};
          
          // Hanya hapus data kontrak yang perlu diisi ulang, biarkan data lainnya
          userData.no_kontrak = '';
          userData.kontrak_akhir = '';
          // Kontrak awal bisa dipertahankan jika perlu
          // userData.kontrak_awal = '';
          
          // Hapus alasan non-aktif
          userData.sebab_na = null;
          
          // Persiapkan pesan yang informatif untuk tampilan edit
          let instructions = 'To reactivate the employee, you MUST:';
          instructions += '\n1. Enter a new contract number';
          instructions += '\n2. Enter the contract end date (must be after today)';
          ...
          setTimeout(() => {
            // Arahkan ke halaman edit dengan data yang lengkap
            navigate('/tambah-user-elnusa', {
              state: {
                ...
                restored: true,
                fieldsToUpdate: ['no_kontrak', 'kontrak_akhir'] // Spesifik field yang perlu diperbarui
              }
            });
          }, 1500);
        })
        .catch(err => {
          ...
          setMessage({
            text: `Failed to restore employee: ${err.response?.data?.message || err.message}`,
            type: "error"
          });
        })
        .finally(() => {
  ...
  const handleDeleteSelected = () => {
    if (selectedRows.length === 0) {
      setMessage({ text: 'Please select at least one item to delete', type: 'error' });
      return;
    }
    ...
    if (window.confirm(`Are you sure you want to permanently delete ${selectedRows.length} selected items?`)) {
      ...
      Promise.all(deletePromises)
        .then(() => {
          setMessage({ text: `${selectedRows.length} items successfully deleted!`, type: 'success' });
          setSelectedRows([]);
          setTimeout(() => setMessage({ text: '', type: '' }), 3000);
        })
        .catch(err => {
          setMessage({ text: `Failed to delete some items: ${err.message}`, type: 'error' });
        });
    }
  };
  ...
  const handleDeleteAll = () => {
    ...
    if (window.confirm(`Are you sure you want to permanently delete all ${filteredRecords.length} items?`)) {
      const deletePromises = filteredRecords.map(row => deleteUserFromBackend(row.id));
      
      Promise.all(deletePromises)
        .then(() => {
          setMessage({ text: `${filteredRecords.length} items successfully deleted!`, type: 'success' });
          setTimeout(() => setMessage({ text: '', type: '' }), 3000);
        })
        .catch(err => {
          setMessage({ text: `Failed to delete some items: ${err.message}`, type: 'error' });
  ...
  const handleExportAllData = () => {
    ...
    exportToExcel(filteredRecords, 'all_nonactive_employee_data');
  };

  // Handler untuk tombol export selected data
  const handleExportSelectedData = () => {
    if (selectedRows.length === 0) {
      setMessage({ text: 'Select at least one data row to export', type: 'error' });
      return;
    }
    exportToExcel(selectedRows, 'selected_nonactive_employee_data');
  };
  ...
  const columns = [
      ...
      cell: row => {
        ...
        return (
          <div className={`px-3 py-1 rounded-full ${getSebabNABadgeColor(sebabNA)}`}>
            <div className="font-medium">{sebabNA}</div>
          </div>
        );
      },
      sortable: true,
    ...
    { name: 'Name',  selector: row => row.nama_karyawan, sortable: true, width: "auto" },
    ...
    { name: 'Vendor NIK', selector: row => row.nik_vendor, sortable: true, width: "120px" },
    { name: 'Elnusa NIK', selector: row => row.nik_elnusa, sortable: true, width: "120px" },
        ...
        <main className="flex-1">
          ...
          <div className='flex-1 flex flex-col text-center overflow-y-auto px-6 py-3'>
            <div className="mb-4 p-4 rounded bg-blue-100 text-blue-700">
              <p className="font-bold">NONACTIVE Employees Page</p>
              <p>This page displays all employees who have a non-active reason or whose contracts have ended.</p>
              <p>To reactivate an employee, click the Restore button and update the contract date and contract number.</p>
            </div>
          ...
          {message.text && (
            ...
            <div className='flex justify-between items-center mb-4'>
              ...
              <div className='relative w-64'>
                ...
                <input type="text" value={searchTerm} onChange={handleFilter} placeholder="Cari..." className='block w-full pl-10 pr-3 py-2 border border-gray-300 rounded-lg 
            ...
            {loading ? (
              <div className="p-6 text-center">
                <p className="text-gray-500">Loading nonactive employee data...</p>
              </div>
            ) : error ? (
              ...
              <DataTable 
                ...
                noDataComponent={
                  <div className="p-4 text-center text-gray-500">
                    No NONACTIVE employee data available.
                  </div>
                }
              />
            )}
...
export default ElnusaNAProjectPage