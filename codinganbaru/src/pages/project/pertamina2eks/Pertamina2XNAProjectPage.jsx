...
const Pertamina2XNAProjectPage = () => {
// State untuk data
...
const fetchNonActiveData = async () => {
  ...
  try {
    ...
    try {
      const checkResult = await axios.get(`${API_BASE_URL}/regional2x/check-expired`, {
        headers: {
    ...
    const response = await axios.get(`${API_BASE_URL}/regional2x/na`, {
      headers: {
        'Accept': 'application/json',
    ...
    if (expiredWithoutSebabNA.length > 0) {
      ...
      // Update sebab_na for these employees
      for (const employee of expiredWithoutSebabNA) {
        try {
          await axios.put(`${API_BASE_URL}/regional2x/users/${employee.id}/na`, 
            { sebab_na: 'EOC' },
            {
              headers: {
                'Content-Type': 'application/json',
      ...
      if (expiredWithoutSebabNA.length > 0) {
        const refreshResponse = await axios.get(`${API_BASE_URL}/regional2x/na`, {
          headers: {
            'Accept': 'application/json'
          },
          params: {
    ...
    // If failed to fetch from /na endpoint, try with backup method
    try {
      const backupResponse = await axios.get(`${API_BASE_URL}/regional2x/users`, {
        headers: {
          'Accept': 'application/json'
        }
      });
...
const manualCheckExpired = async () => {
  try {
    ...
    const updatePromises = expiredNoSebabNA.map(user => 
      axios.put(`${API_BASE_URL}/regional2x/users/${user.id}/na`, 
        { sebab_na: 'EOC' },
        {
          headers: {
            'Content-Type': 'application/json',
...
const deleteUserFromBackend = async (id) => {
  try {
    ...
    const response = await axios.delete(`${API_BASE_URL}/regional2x/users/${id}`, {
      headers: {
        'Accept': 'application/json'
      }
    });
...
const restoreUserToActive = async (id) => {
  try {
    ...
    const response = await axios.put(
      `${API_BASE_URL}/regional2x/users/${id}/restore`, 
      {},
      {
        headers: {
          'Content-Type': 'application/json',
...
- The contract end date must be in the future`;
  ...
  if (window.confirm(confirmMessage)) {
    ...
    try {
      ...
      axios.put(
        `${API_BASE_URL}/regional2x/users/${user.id}/restore`,
        {},
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      .then(response => {
        ...
        setTimeout(() => {
          ...
          navigate('/tambah-user-regional2x', {
            state: {
              editMode: true,
              userData: userData,
              projectId: 'regional2x',
              message: instruksi,
              restored: true
            }
          });
...
const getDaysSinceDeactivation = (updatedAt) => {
  ...
  const columns = [
    ...
    { name: 'Name',  selector: row => row.nama_lengkap, sortable: true, width: "auto" },
...
export default Pertamina2XNAProjectPage