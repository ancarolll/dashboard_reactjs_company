...
const FormElnusaHSEEdit = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    nik_elnusa: '',
    tanggal_lahir: '',
    hse: 'RIG EMR', // Ensure HSE value stays as RIG EMR
  });
  ...
  const fetchEmployeeData = async () => {
    ...
    try {
      const response = await axios.get(`${API_BASE_URL}/elnusa/users/${id}`, {
        headers: {
      ...
      if (response.data) {
        ...
        // Verify that this is a RIG EMR employee
        if (employeeData.hse !== 'RIG EMR') {
          setError('This user is not in the RIG EMR category. Only users with HSE status "RIG EMR" can be edited.');
          setLoading(false);
          return;
        }
        ...
        setFormData({
          ...
          nik_elnusa: employeeData.nik_elnusa || '',
          alamat_lengkap_domisili: employeeData.alamat_lengkap_domisili || '',
          tanggal_lahir: formatDateForInput(employeeData.tanggal_lahir),
          hse: employeeData.hse || 'RIG EMR',
        });
...
// Fetch HSE history data
  const fetchHSEHistory = async (userId) => {
    ...
    try {
      ...
      const response = await axios.get(
        `${API_BASE_URL}/elnusa/users/${userId}/hse-history?t=${timestamp}`,
        {
          headers: {
            'Accept': 'application/json',
            'Content-Type': 'application/json'
  ...
  const downloadHSEHistory = () => {
    ...
    link.setAttribute('download', `hse-history-${formData.nama_karyawan || id}-Elnusa-RIG-EMR.csv`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Ensure HSE field is preserved as RIG EMR
      const dataToSubmit = {
        ...formData,
        hse: 'RIG EMR',
      ...
      // Use the HSE-specific endpoint for update
      const response = await axios.put(
        `${API_BASE_URL}/elnusa/users/${id}/hse`,
        dataToSubmit,
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if (response.data) {
        setMessage({
          text: `Successfully updated HSE data for ${formData.nama_karyawan}`,
          type: 'success'
        });
        ...
        setTimeout(() => {
          navigate('/hse-elnusa', {
            state: {
              ...
              needRefresh: true // Flag untuk trigger refresh
            }
          });
        }, 1500);
      }
  ...
  useEffect(() => {
    if (location.state?.userData) {
      ...
      // Verify this is a RIG EMR user
      if (userData.hse !== 'RIG EMR') {
        setError('This user is not in the RIG EMR category. Only users with HSE status "RIG EMR" can be edited.');
        return;
      }
      ...
      setFormData({
        ...
        nik_elnusa: userData.nik_elnusa || '',
      ...
      <div className="flex">
        ...
        <div className="flex-1 flex flex-col h-screen ml-68 bg-(--background-tar-color) overflow-y-auto px-6 py-3">
          ...
          <main className="flex-1">
            <h1 className="text-2xl font-bold mb-4 mt-2 text-center">Edit HSE Data Elnusa RIG EMR</h1>
            ...
            {loading ? (
              ...
              <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-md p-6">
                ...
                <div className="mb-6 border-b pb-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <div className="mb-4">
                      ...
                      <input 
                        type="text"
                        className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight bg-green-100 font-semibold"
                        value={formData.hse}
                        readOnly
                      />
                    </div>
                ...
                <div className="flex justify-end mt-6">
                  <Link to="/hse-elnusa" className="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded mr-2">
                    Cancel
                  </Link>
                  <button 
                    type="submit" 
            ...
            {/* HSE History Section - Integrated directly into the main component */}
...
export default FormElnusaHSEEdit