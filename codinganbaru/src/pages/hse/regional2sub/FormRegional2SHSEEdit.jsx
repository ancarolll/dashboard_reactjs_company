...
const FormRegional2SHSEEdit = () => {
  ...
  try {
    // Jika sudah dalam format YYYY-MM-DD atau dengan timestamp
  ...
  const fetchEmployeeData = async () => {
    ...
    try {
      const response = await axios.get(`${API_BASE_URL}/regional2s/users/${id}`, {
        headers: {
  ...
  const fetchHSEHistory = async (userId) => {
    ...
    try {
      ...
      const response = await axios.get(
        `${API_BASE_URL}/regional2s/users/${userId}/hse-history?t=${timestamp}`,
        {
          headers: {
            'Accept': 'application/json',
            'Content-Type': 'application/json'
  ...
  const downloadHSEHistory = () => {
    ...
    link.setAttribute('download', `hse-history-${formData.nama_lengkap || id}-Pertamina EP Regional 2 Subsurface.csv`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(
        `${API_BASE_URL}/regional2s/users/${id}/hse`,
        formattedData,
        {
          headers: {
            'Content-Type': 'application/json',
  ...
  useEffect(() => {
    if (location.state?.needRefresh) {
      ...
      navigate('/hse-regional2s', { replace: true });
    }
      ...
      <div className="flex">
        ...
        <div className="flex-1 flex flex-col h-screen ml-68 bg-(--background-tar-color) overflow-y-auto px-6 py-3">
          ...
          <main className="flex-1">
            <h1 className="text-2xl font-bold mb-4 mt-2 text-center">Edit HSE Data Pertamina EP Regional 2 Subsurface</h1>
            ...
            {loading ? (
              ...
              <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-md p-6">
                ...
                <div className="flex justify-end mt-6">
                  <Link to="/hse-regional2s" className="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded mr-2">
                    Cancel
                  </Link>
                  <button 
                    type="submit" 
            ...
            {/* HSE History Section - Integrated directly into main component */}
...
export default FormRegional2SHSEEdit