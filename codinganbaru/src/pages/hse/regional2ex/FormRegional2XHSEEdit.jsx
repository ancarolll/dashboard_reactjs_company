...
const FormRegional2XHSEEdit = () => {
  ...
  const fetchEmployeeData = async () => {
    ...
    try {
      const response = await axios.get(`${API_BASE_URL}/regional2x/users/${id}`, {
        headers: {
      ...
      if (response.data) {
        ...
        setFormData({
          ...
          nik_vendor: employeeData.nik_vendor || '',
  ...
  const fetchHSEHistory = async (userId) => {
    ...
    try {
      ...
      const response = await axios.get(
        `${API_BASE_URL}/regional2x/users/${userId}/hse-history?t=${timestamp}`,
        {
          headers: {
            'Accept': 'application/json',
            'Content-Type': 'application/json'
  ...
  const downloadHSEHistory = () => {
    ...
    link.setAttribute('download', `hse-history-${formData.nama_lengkap || id}-Pertamina EP Regional 2 Eksplorasi.csv`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(
        `${API_BASE_URL}/regional2x/users/${id}/hse`,
        formattedData,
        {
          headers: {
            'Content-Type': 'application/json',
  ...
  useEffect(() => {
    if (location.state?.userData) {
      ...
      setFormData({
        ...
        nik_vendor: userData.nik_vendor || '',
        no_ktp: userData.no_ktp || '',
        tempat_tanggal_lahir: userData.tempat_tanggal_lahir || '',
      });
  ...
  useEffect(() => {
    if (location.state?.needRefresh) {
      // Fetch data again if returning from edit page
      fetchEmployeeData();
      
      // Clear state to prevent continuous refresh
      navigate('/hse-regional2x', { replace: true });
    }
    
    // If there's a successful edit message, display it
    if (location.state?.editedUser && location.state?.updatedUser) {
      setMessage({
        text: `HSE data for ${location.state.updatedUser.nama_lengkap} has been successfully updated`,
        type: 'success'
      });
    }
      ...
      <div className="flex">
        ...
        <div className="flex-1 flex flex-col h-screen ml-68 bg-(--background-tar-color) overflow-y-auto px-6 py-3">
          ...
          <main className="flex-1">
            <h1 className="text-2xl font-bold mb-4 mt-2 text-center">Edit HSE Data Pertamina EP Regional 2 Eksplorasi</h1>
            ...
            {loading ? (
              ...
              <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow-md p-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="mb-4">
                      ...
                      <input 
                        ...
                        onChange={handleInputChange}
                        placeholder="Masukkan vendor MCU"
                      />
                    </div>
                  </div>
                </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="mb-4">
                      ...
                      <input 
                        ...
                        onChange={handleInputChange}
                        placeholder="Masukkan nomor HSE passport"
                      />
                    </div>
                ...
                <div className="flex justify-end mt-6">
                  <Link to="/hse-regional2x" className="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded mr-2">
                    Cancel
                  </Link>
                  <button 
                    type="submit" 
...
export default FormRegional2XHSEEdit