...
import TarSection from '../../components/manajemencom/TarSection';
import { useLocation, useNavigate } from 'react-router-dom'
import DataTable from 'react-data-table-component';
import ManagementService from '../../service/managementService';
import { toast } from 'react-toastify';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faSearch, faFileDownload, faPenToSquare, faTrashAlt, faFile } from '@fortawesome/free-solid-svg-icons';

const EditManagementPage = ({ onLogout }) => {
    ...
    // Fungsi untuk memuat dokumen dari API
    ...
    // Fungsi untuk memuat section dari API
    ...
    // Check for updates
    useEffect(() => {
        if (location.state?.addedDocument || location.state?.editedDocument) {
            loadDocuments();
        }
    }, [location.state, loadDocuments]);
    ...
    const handleEdit = (id) => {
        navigate(`/add-management/${id}`, { state: { editMode: true, documentId: id } });
    };
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Are you sure you want to delete this document?')) {
            try {
                const response = await ManagementService.deleteDocument(id);

                if (response.success) {
                    toast.success('Document deleted successfully');
                    loadDocuments();
                } else {
                    toast.error(response.message || 'Error deleting document');
                }
            } catch (error) {
                            ...
                            <div className="mt-4">
                                <div className="flex justify-between items-center mb-4">
                                    <h4 className="text-lg font-semibold">
                                        {sectionFilter === 'all' ? 'All Document' : sectionFilter}
                                    </h4>
                                    <div className="text-sm text-gray-500">
                                        Total: {totalEntries} document
                                    </div>
                                </div>
                                ...
                                {loading ? (
                                    ...
                                    <div className="text-center py-8 text-gray-500 border rounded-md">
                                        ...
                                        <p className="text-sm mt-2">Please add a new document or modify the search filter</p>
                                    </div>
                                ) : (
                                    <DataTable
                                        columns={columns}
                                        ...
                                        conditionalRowStyles={[
                                            {
                                                ...
                                                style: {
                                                    backgroundColor: 'rgba(254, 202, 202, 0.5)', // Merah muda untuk dokumen kadaluarsa
                                                },
                                            },
                                            {
                                                when: row => {
                                                ...
                                                style: {
                                                    backgroundColor: 'rgba(254, 240, 138, 0.5)', // Kuning untuk reminder 2
                                                },
                                            },
                                            {
                                                when: row => {
                                                ...
                                                style: {
                                                    backgroundColor: 'rgba(187, 247, 208, 0.5)', // Hijau muda untuk reminder 1
                                                },
                                            },
                                        ]}
                                        pagination
