...
export class Reg2z7dBudgetController {
  // Get master budget and all projects
  static async getBudgetData(req, res) {
    try {
      // Get master budget
      const masterBudget = await Reg2z7dBudgetModel.getMasterBudget();
      ...
      const projects = await Reg2z7dBudgetModel.getAllProjects(
        filters, 
        parseInt(page), 
        parseInt(limit)
      );
  ...
  static async createOrUpdateMaster(req, res) {
    try {
      ...
      // Create or update master budget
      const result = await Reg2z7dBudgetModel.createOrUpdateMaster(budgetData, username);
  ...
  static async addOrUpdateProject(req, res) {
    try {
      ...
      // Add or update project
      const result = await Reg2z7dBudgetModel.addOrUpdateProject(projectData, username);
  ...
  static async deleteProject(req, res) {
    try {
      ...
      // Delete project
      const result = await Reg2z7dBudgetModel.deleteProject(parseInt(id), username);
  ...
  static async getBudgetHistory(req, res) {
    try {
      const history = await Reg2z7dBudgetModel.getBudgetHistory();
  ...
  static async getFilterOptions(req, res) {
    try {
      const options = await Reg2z7dBudgetModel.getFilterOptions();
  ...
  static async getBudgetSummary(req, res) {
    try {
      const summary = await Reg2z7dBudgetModel.getBudgetSummary();
  ...
  static async importExcel(req, res) {
    try {
      ...
      const result = await Reg2z7dBudgetModel.importFromExcel(mappedData, username);
  ...
  static async exportExcel(req, res) {
    try {
        ...
        const fileName = `reg2z7d_budget_export_${timestamp}.xlsx`;
...
export default Reg2z7dBudgetController;