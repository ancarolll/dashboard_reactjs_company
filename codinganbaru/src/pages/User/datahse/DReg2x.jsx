...
import { faHouse, faFileContract, faShieldHalved, faArrowRightFromBracket, faBell, faSun, 
faMagnifyingGlass, faCalendarAlt, faIdCard, faCarAlt } from '@fortawesome/free-solid-svg-icons';
...
const DReg2x = () => {
  ...
  // Effect for setting active path
  useEffect(() => {
  ...
  const getDaysDifference = (endDate) => {
    if (!endDate) return null; // Return null for dates not yet filled
  ...
  const getFilteredData = (data, searchTerm, filterType, dateField) => {
    ...
    if (filterType === 'all') {
      // If there's a search, filter based on the search
      if (searchTerm) {
    ...
    let statusFiltered = data.filter(employee => {
      ...
      if (!employee[dateField]) {
        // For 'nonactive', also show those without dates
  ...
  const simlFilteredData = useMemo(() => {
  ...
  const customStyles = {
    table: {
      style: {
        width: '100%',
        tableLayout: 'auto', // Let browser determine table layout automatically
      }
    },
    rows: { 
      style: { 
        minHeight: '60px',
        width: '100%' // Ensure rows take full available width
      },
    ...
    headCells: {
      ...
      activeSortStyle: {
        color: '#ffdd99', // Lighter color for sorted columns
        '&:focus': {
          outline: 'none',
        },
      },
      ...
      <div className="flex">
          ...
          <div className='flex sticky top-0'>
            <aside className="w-70 bg-white text-black-70 p-4 flex flex-col h-screen overflow-y-auto">
              ...
              <nav className="mt-8 space-y-4">
                {/* Link Dashboard */}
                <Link to="/user-reg2x" className={getLinkClassName('/user-reg2x')}>
                  <FontAwesomeIcon icon={faHouse} className='text-lg'/>
                  <span className='text-sm'>Dashboard</span>
                </Link>
                
                {/* Link Contract */}
                <Link to="/contract-reg2x" className={getLinkClassName('/contract-reg2x')}>
                  <FontAwesomeIcon icon={faFileContract} className='text-lg'/>
                  <span className='text-sm'>Contract</span>
                </Link>
                
                {/* Link HSE */}
                <Link to="/data-hse-reg2x" className={getLinkClassName('/data-hse-reg2x')}>
        ...
        <div className="flex-1 flex flex-col h-screen ml-68 bg-(--background-tar-color) overflow-y-auto px-6 py-3">
          ...
          <main className="flex-1">
            <div className="mb-4">
              <Link to='/user-reg2x' className="inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition">
              &lt; Back
              </Link>
            </div>
