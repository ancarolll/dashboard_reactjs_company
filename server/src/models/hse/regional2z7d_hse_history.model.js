...
export const Regional2z7dHSEHistory = {
    /**
    ...
    ensureTableExists: async () => {
        ...
        const checkTableSql = `
            ...
            AND table_name = 'hse_regional2z7d_history'
            );
        `;
        ...
        const createTableSql = `
            CREATE TABLE hse_regional2z7d_history (
            ...
            FOREIGN KEY (user_id) REFERENCES project_pertamina_ep_regional2zona7_devplan(id) ON DELETE CASCADE
            );
        `;
    ...
    addHistory: async (historyData) => {
        try {
        // Ensure table exists
        await Regional2z7dHSEHistory.ensureTableExists();
        ...
        const sql = `
            INSERT INTO hse_regional2z7d_history (${keys.join(', ')})
            VALUES (${placeholders})
            RETURNING *
        `;
    ...
    getAllHistoryByUserId: async (userId) => {
        ...
        const sql = `
            SELECT * FROM hse_regional2z7d_history
            WHERE user_id = $1
    ...
    prepareHistoryData: (oldData, newData, modifiedBy = 'system') => {
        return {
        user_id: oldData.id,
        nama_karyawan: oldData.nama_lengkap,
