...
export const Regional2Z7WHSEHistory = {
    /**
    ...
    ensureTableExists: async () => {
        ...
        const checkTableSql = `
            ...
            AND table_name = 'hse_regional2z7w_history'
            );
        `;
        ...
        const createTableSql = `
            CREATE TABLE hse_regional2z7w_history (
            ...
            FOREIGN KEY (user_id) REFERENCES project_pertamina_ep_regional2zona7_wopdm(id) ON DELETE CASCADE
            );
        `;
    ...
    addHistory: async (historyData) => {
        try {
        // Ensure table exists
        await Regional2Z7WHSEHistory.ensureTableExists();
        ...
        const sql = `
            INSERT INTO hse_regional2z7w_history (${keys.join(', ')})
            VALUES (${placeholders})
            RETURNING *
        `;
    ...
    getAllHistoryByUserId: async (userId) => {
        ...
        const sql = `
            SELECT * FROM hse_regional2z7w_history
            WHERE user_id = $1
