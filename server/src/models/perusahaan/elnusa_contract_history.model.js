...
export const ElnusaContractHistory = {
  /**
  ...
  addHistory: async (historyData) => {
    try {
      ...
      const query = `
        INSERT INTO elnusa_contract_history 
        (${keys.join(', ')})
        VALUES (${placeholders})
        RETURNING *
      `;
    ...
    try {
      ...
      const checkTableQuery = `
        SELECT EXISTS (
          ...
          AND table_name = 'elnusa_contract_history'
        );
      `;
      ...
      if (!tableExists.rows[0].exists) {
        console.error("Table elnusa_contract_history does not exist");
        return [];
      }
      
      // Use parameterized query with better formatting
      const query = `
        SELECT * FROM elnusa_contract_history
        WHERE user_id = $1
      ...
      if (result.rowCount === 0) {
        const diagnosticQuery = `SELECT COUNT(*) FROM elnusa_contract_history`;
        ...
        if (parseInt(diagnosticResult.rows[0].count) > 0) {
          ...
          const sampleQuery = `SELECT * FROM elnusa_contract_history LIMIT 1`;
