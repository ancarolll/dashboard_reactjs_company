...
export const ElnusaHSEHistory = {
    ...
    ensureTableExists: async () => {
        ...
        const checkTableSql = `
            ...
            AND table_name = 'hse_elnusa_history'
            );
        `;
        ...
        const createTableSql = `
            CREATE TABLE hse_elnusa_history (
            ...
            FOREIGN KEY (user_id) REFERENCES project_elnusa(id) ON DELETE CASCADE
            );
        `;
    ...
    addHistory: async (historyData) => {
        try {
        // Ensure table exists
        await ElnusaHSEHistory.ensureTableExists();
        ...
        const sql = `
            INSERT INTO hse_elnusa_history (${keys.join(', ')})
            VALUES (${placeholders})
            RETURNING *
        `;
    ...
    getAllHistoryByUserId: async (userId) => {
        ...
        const sql = `
            SELECT * FROM hse_elnusa_history
            WHERE user_id = $1
