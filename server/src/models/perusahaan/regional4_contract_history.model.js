...
export const Regional4ContractHistory = {
    /**
    ...
    addHistory: async (historyData) => {
        try {
            ...
            const query = `
            INSERT INTO regional4_contract_history 
            (${keys.join(', ')})
            VALUES (${placeholders})
            RETURNING *
            `;
        ...
        try {
            ...
            SELECT EXISTS (
                ...
                AND table_name = 'regional4_contract_history'
            );
            `;
            ...
            console.error("Table regional4_contract_history does not exist");
            return [];
            }
            
            // Use parameterized query with better formatting
            const query = `
            SELECT * FROM regional4_contract_history
            WHERE user_id = $1
            ...
            const diagnosticQuery = `SELECT COUNT(*) FROM regional4_contract_history`;
            ...
            if (parseInt(diagnosticResult.rows[0].count) > 0) {
                ...
                const sampleQuery = `SELECT * FROM regional4_contract_history LIMIT 1`;
        ...
        try {
            ...
            const newHistoryQuery = `
                ...
                FROM regional4_contract_history
                WHERE user_id = $1
            `;
