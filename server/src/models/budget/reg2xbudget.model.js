...
export class Reg2xBudgetModel {
    ...
    try {
      ...
      const masterTableCheck = await pool.query(`
        SELECT EXISTS (
          ...
          WHERE table_name = 'reg2x_budget_master'
        );
      `);
      ...
      if (!masterTableCheck.rows[0].exists) {
        await pool.query(`
          CREATE TABLE reg2x_budget_master (
          ...
          CREATE TABLE IF NOT EXISTS reg2x_budget_absorption (
            id SERIAL PRIMARY KEY,
          ...
          CREATE TABLE IF NOT EXISTS reg2x_budget_history (
            id SERIAL PRIMARY KEY,
          ...
          CREATE INDEX IF NOT EXISTS idx_reg2x_budget_absorption_period ON reg2x_budget_absorption(period);
        `);
        console.log('Reg2x Budget tables created successfully');
      }
    } catch (error) {
      console.error('Error initializing reg2x_budget tables:', error);
      throw error;
    }
  }
  ...
  static async createOrUpdateMaster(budgetData, username = 'system') {
    try {
      ...
      try {
        ...
        const checkQuery = `SELECT * FROM reg2x_budget_master`;
        ...
        if (checkResult.rows.length === 0) {
          ...
          const insertQuery = `
            INSERT INTO reg2x_budget_master (
          ...
          const updateQuery = `
            UPDATE reg2x_budget_master SET
              total_budget = $1,
              current_remaining = $2,
              status = $3,
              remarks = $4,
          ...
          if (oldTotalBudget !== newTotalBudget) {
            await client.query(`
              INSERT INTO reg2x_budget_history (
  ...
  static async addPeriodAbsorption(absorptionData, username = 'system') {
    try {
      ...
      try {
        ...
        const masterQuery = `SELECT * FROM reg2x_budget_master`;
        ...
        // Check if absorption is valid
        if (absorptionAmount > currentRemaining) {
          throw new Error('Absorption amount exceeds remaining budget');
        }
        ...
        const periodCheckQuery = `
          SELECT * FROM reg2x_budget_absorption WHERE period = $1
        `;
        ...
        if (periodCheckResult.rows.length === 0) {
          ...
          const insertQuery = `
            INSERT INTO reg2x_budget_absorption (
          ...
          if (currentRemaining - absorptionDifference < 0) {
            throw new Error('Updated absorption would exceed remaining budget');
          }
          
          const updateQuery = `
            UPDATE reg2x_budget_absorption SET
              absorption_amount = $1,
              remaining_after = $2,
              remarks = $3,
              updated_at = CURRENT_TIMESTAMP
        ...
        if (periodCheckResult.rows.length > 0) {
          await client.query(`
            UPDATE reg2x_budget_absorption
  ...
  static async getMasterBudget() {
    try {
      const query = `SELECT * FROM reg2x_budget_master`;
  ...
  static async getAllAbsorptions(filters = {}, page = 1, limit = 50) {
    try {
      let query = `SELECT * FROM reg2x_budget_absorption WHERE 1=1`;
      const values = [];
      let paramCount = 1;
      ...
      let countQuery = `SELECT COUNT(*) FROM reg2x_budget_absorption WHERE 1=1`;
      let countValues = [];
  ...
  // Get absorption by ID
  static async getAbsorptionById(id) {
    try {
      const query = `SELECT * FROM reg2x_budget_absorption WHERE id = $1`;
      ...
      console.error(`Error getting absorption with ID ${id}:`, error);
      throw error;
    }
  }
  ...
  static async deleteAbsorption(id, username = 'system') {
    try {
      ...
      try {
        ...
        const absorptionQuery = `SELECT * FROM reg2x_budget_absorption WHERE id = $1`;
        ...
        const deleteQuery = `DELETE FROM reg2x_budget_absorption WHERE id = $1 RETURNING *`;
  ...
  static async getBudgetHistory() {
    try {
      const query = `
        SELECT * FROM reg2x_budget_history 
  ...
  static async getFilterOptions() {
    try {
      const periodQuery = `SELECT DISTINCT period FROM reg2x_budget_absorption ORDER BY period`;
  ...
  static async getBudgetSummary() {
    try {
      ...
      const absorptionsQuery = `SELECT * FROM reg2x_budget_absorption ORDER BY created_at`;
    ...
    try {
      ...
      try {
        ...
        if (data.length > 0) {
          ...
          const masterQuery = `
            SELECT * FROM reg2x_budget_master
          `;
        ...
        const updatedMasterResult = await client.query(`SELECT * FROM reg2x_budget_master`);
        ...
        for (let i = 1; i < data.length; i++) {
          ...
          // Check if absorption is valid
          if (absorptionAmount > currentRemaining) {
            throw new Error(`Absorption amount for ${absorptionData.period} exceeds remaining budget`);
          }
...
export default Reg2xBudgetModel;